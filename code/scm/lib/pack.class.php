...
function get_partial_color($order,$partial_id){
	//這邊要組成HTML語法，回傳
	$sql = $this->sql;
	$t_sql = "select wiqty.colorway from order_partial ";
	$t_sql .= "left join wiqty on order_partial.id=wiqty.p_id ";
	$t_sql .= "where order_partial.ord_num='";
	$t_sql .= $order;
	...
	$arraylength = sizeof($partial_color);
	foreach($partial_color as $key => $val)
	{
		//$back .= $val['colorway'].',';
		//$back .= $key.',';
