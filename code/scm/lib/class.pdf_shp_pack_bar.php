...
class PDF_shp_pack_bar extends PDF_Chinese 	{
...
{
	//290
	$fld_size = 120 / sizeof($size);
	
	$this->SetFont('Arial','B',9);
	$this->SetX(8);
	$this->Cell(14,4,'BAR#','TLR',0,'C');
	$this->Cell(30,4,'ROPE NO.','TLR',0,'C');
	$this->Cell(35,4,'','TLR',0,'C');
	$this->Cell(35,4,'','TLR',0,'C');
	$this->Cell(0.3,4,'','TLR',0,'C');
	$this->Cell(120,6,'RATIO OR CONTENT PER CARTON','TRL',0,'C');
	$this->Cell(0.3,4,'','TLR',0,'C');
	...
	$this->Cell(35,4,'STYLE','LR',0,'C');
	$this->Cell(35,4,'COLOR','LR',0,'C');
...
function ship_txt($parm,$in_box)
{
	...
	if($parm['f_mk'] == 1)
	{
		$this->Cell(14,4,$parm['ctn_mk'],'TLR',0,'C');
	...
	if($parm['ord_row'])
	{
		$this->Cell(35,4,$parm['style_num'],'TLR',0,'C');
	}else{
		$this->Cell(35,4,$parm['style_num'],'LR',0,'C');
	}
	if($parm['color_row'])
	{
		$this->Cell(35,4,$parm['color'],'TLR',0,'C');
		$this->Cell(0.3,4,'','TLR',0,'C');
	}else{
		$this->Cell(35,4,$parm['color'],'LR',0,'C');
		$this->Cell(0.3,4,'','LR',0,'C');
	}
...
function ship_txt_mix($parm)
{
	...
	$this->Cell(35,4,substr($parm['style_num'],0,22),'TLR',0,'C');
	$this->Cell(35,4,substr($parm['color'],0,22),'TLR',0,'C');
	$this->Cell(0.3,4,'','TLR',0,'C');
	$this->SetFont('Arial','',8);
...
//²Ä¤G¦C	
	$this->Cell(14,4,'','BLR',0,'C');
	$this->Cell(15,4,'','BLR',0,'C');
	$this->Cell(15,4,'','BLR',0,'C');
	$this->Cell(35,4,substr($parm['style_num'],22),'BLR',0,'C');
	$this->Cell(35,4,substr($parm['color'],22),'BLR',0,'C');
	$this->SetFont('Arial','',8);
	$this->Cell(0.3,4,'','BLR',0,'C');
...
function sub_total($parm)
{
	...
	$this->SetFont('Arial','',8);
	$this->SetX(8);
	$this->Cell(114,0.5,'',1,0,'C');
	$this->Cell(0.3,0.5,'',1,0,'C');
	
	$this->Cell(120,0.5,'',1,0,'C');
	$this->Cell(0.3,0.5,'',1,0,'C');
	...
	$this->SetX(8);
	$this->Cell(114,4,'SUB-TOTAL >>',1,0,'C');
	$this->Cell(0.3,4,'',1,0,'C');
...
function gd_total($parm)
{
	...
	$this->SetX(8);
	$this->Cell(114,4,'',1,0,'C');
	$this->Cell(0.3,4,'',1,0,'C');
	
	$this->Cell(120,4,'',1,0,'C');
	$this->Cell(0.3,4,'',1,0,'C');
	...
	$this->SetX(8);
	$this->Cell(114,4,'GRAND TOTAL >>',1,0,'C');
	$this->Cell(0.3,4,'',1,0,'C');
	for ($i=0; $i<sizeof($parm['size']); $i++)$this->Cell($fld_size,4,$parm['size'][$i],1,0,'C');
	$this->Cell(0.3,4,'',1,0,'C');
	$this->Cell(18,4,$parm['ctn'],1,0,'C');
