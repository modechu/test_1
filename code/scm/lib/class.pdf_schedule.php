...
class PDF_schedule extends PDF_Chinese 	{
...
function Header() {
	...
	$this->Cell(125, 7,$print_title,1,0,'C',0);
    $this->SetFont('Arial','I',12);
	$this->Cell(50, 7,$print_title2,0,0,'R',0);
...
//	$this->Cell(20, 7,$dt['date'],0,1,'C',0);
	...
	$Y = $this->getY();
	$this->setXY(3,$Y+5);
	$this->hend_title($days);
	$Y = $this->getY();
	$this->setXY(3,$Y+7);
	
}

function hend_title($sch_day)
{
 $c_w = 280 / $sch_day;
 $this->Cell(12, 7,'LN',1,0,'C');
 for($i=1; $i <= $sch_day; $i++)
 {
 	$this->Cell($c_w, 7,$i,1,0,'C'); 	
 }
...
// field module 1 [表匡 1 ]
function thd_line_sch($rec,$sch_day) {
	$style= '';
	$c_w = 280 / $sch_day;
	if($rec['style'] == 'top') $style = '(T)';
	if($rec['style'] == 'btm.') $style = '(B)';
//第一列	
		$this->setX(3);
		$this->SetFont('Arial','B',10);
		$this->Cell(12, 6,$rec['line'],'TLR',0,'C');
		$this->SetFont('Arial','',8);

		for($i=0; $i<sizeof($rec['sch_rec']); $i++)
		{
			$ord_w = $c_w * $rec['sch_rec'][$i]['day_range'];
			if(!isset($rec['sch_rec'][$i]['ord_num']))$rec['sch_rec'][$i]['ord_num'] = '-';
			$ord_num = explode('-',$rec['sch_rec'][$i]['ord_num']);
			
			$this->Cell($ord_w, 4,$ord_num[1],'TLR',0,'C');			
		}
...
//第二列	
  	$this->setX(3);
  	$this->SetFont('Arial','B',10);
		$this->Cell(12, 7,$style,'LR',0,'C');
		$this->SetFont('Arial','',8);
		for($i=0; $i<sizeof($rec['sch_rec']); $i++)
		{
			$ord_w = $c_w * $rec['sch_rec'][$i]['day_range'];
			if(!isset($rec['sch_rec'][$i]['style']))
			{
				$rec['sch_rec'][$i]['style'] = '';
			}else{
				$rec['sch_rec'][$i]['style'] = '['.$rec['sch_rec'][$i]['style'].']';
			}			
			$this->Cell($ord_w, 4,$rec['sch_rec'][$i]['style'],'LR',0,'C');			
		}
		$this->ln();
...
//第三列
  	...
  	$this->SetFont('Arial','B',10);
		...
		{
			$ord_w = $c_w * $rec['sch_rec'][$i]['day_range'];
			if(!isset($rec['sch_rec'][$i]['etd']))
			{
				$etd = '';
			}else{
			 $tmp = explode('-',$rec['sch_rec'][$i]['etd']);
			 $etd = $tmp[1].'/'.$tmp[2];
			}						
			$this->Cell($ord_w, 4,$etd,'BLR',0,'C');			
		}		
		$this->ln();
}

function two_line_sch($rec,$sch_day) {
	$style= '';
	$c_w = 280 / $sch_day;
	$thd_mk = 0;
	if($rec['style'] == 'top') $style = '(T)';
...
//第一列	
		...
		$this->setX(3);
		$this->SetFont('Arial','B',10);
		$this->Cell(12, 4,$rec['line'],'TLR',0,'C');
		$this->SetFont('Arial','',8);
		...
		for($i=0; $i<sizeof($rec['sch_rec']); $i++)
		{
			...
			$this->Cell($ord_w, 4,$ord_num[1]."  ". $rec['sch_rec'][$i]['style'],'TLR',0,'C');			
		}
...
function sc_line_sch($rec,$sch_day) {
	$c_w = 280 / $sch_day;

		$this->setX(3);
		$this->SetFont('Arial','B',10);
		$this->Cell(12, 6,'SC.',1,0,'C');
		$this->SetFont('Arial','',8);

		for($i=0; $i<sizeof($rec['sch_rec']); $i++)
		{
			$ord_w = $c_w * $rec['sch_rec'][$i]['day_range'];
			if(isset($rec['sch_rec'][$i]['ord_num']))
			{	
			  $ord_num = explode('-',$rec['sch_rec'][$i]['ord_num']);
			  $style = '     ['.$rec['sch_rec'][$i]['style'].']';
			  $tmp = explode('-',$rec['sch_rec'][$i]['etd']);
			 	$etd = '     ETD : '.$tmp[1].'/'.$tmp[2];
			 	$sch = '     ETS : '.$rec['sch_rec'][$i]['ets']."  ETF : ".$rec['sch_rec'][$i]['etf'];
			  $this->Cell(280, 6,$ord_num[1].$style.$etd.$sch,1,0,'L');		
			}				
		}	
		$this->ln();
}
