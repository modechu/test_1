...
class PDF_schedule_small extends PDF_Chinese 	{
...
function Header() {
    ...
    $this->RotatedText(20,170,$mark,20);
	//Logo
...
//	$this->Cell(20, 7,$dt['date'],0,1,'C',0);
	...
	$Y = $this->getY();
	$this->setXY(3,$Y+5);
	$this->hend_title($yy_mm,$span_mm,$days);
	$Y = $this->getY();
	$this->setXY(3,$Y);
	
}

function hend_title($yy_mm,$span_mm,$days)
{
 $c_w = 277 / sizeof($days);
 $this->Cell(15, 7,'LN','TLR',0,'C');
 for($i=0; $i < sizeof($span_mm); $i++)
 {
 	$m_w = $c_w * $span_mm[$i];
 	$this->Cell($m_w, 7,$yy_mm[$i],1,0,'C'); 	
 }
 $this->ln();
 $this->setX(3);
 $this->Cell(15, 7,'','BLR',0,'C');
 for($i=0; $i < sizeof($days); $i++)
 {
 	$this->Cell($c_w, 7,$days[$i],1,0,'C'); 	
 }
 $this->ln(); 
}
...
//第一列	
		...
		$this->Cell(15, 5,$rec['line'].$style,1,0,'C');

		for($i=0; $i<sizeof($rec['sch_rec']); $i++)
		{
			if($rec['sch_rec'][$i]['day_range'] == 1)$this->SetFont('Arial','',6);
			if($rec['sch_rec'][$i]['day_range'] > 1)$this->SetFont('Arial','',8);
			$ord_w = $c_w * $rec['sch_rec'][$i]['day_range'];
			if(!isset($rec['sch_rec'][$i]['ord_init']))$rec['sch_rec'][$i]['ord_init'] = '';			
			$this->Cell($ord_w, 5,$rec['sch_rec'][$i]['ord_init'],1,0,'C');			
		}	
		$this->ln();

}

function sc_line_sch($rec) {

		$this->setX(3);
		$this->SetFont('Arial','',10);
		$this->Cell(15, 5,'SC.',1,0,'C');
		...
		for($i=0; $i<sizeof($rec['sch_rec']); $i++)
		{
			...
			{	
			  ...
			  $this->Cell(277, 5,$rec['sch_rec'][$i]['ord_init'].$sch,1,0,'L');		
			}				
		}	
		$this->ln();
}
