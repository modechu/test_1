...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function add($parm) {
		...
		$perm_array = array();
					...
					# 加入資料庫
		$q_str = "INSERT INTO smpl_wiqty (	wi_id, 
										colorway, 
										qty) VALUES('".
										$parm['wi_id']."','".
										$parm['colorway']."','".
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function search($mode=0,$where_str="") {
		...
		if($mode=1){
			$q_header = "SELECT * FROM smpl_wiqty ".$where_str;
			if (!$srh->add_q_header($q_header)) {
				$this->msg->merge($srh->msg);
				return false;
			}
		} else {
			$q_header = "SELECT * FROM smpl_wiqty";
			if (!$srh->add_q_header($q_header)) {
				$this->msg->merge($srh->msg);
				return false;
			}
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function del($id,$mode=0) {
		...
		if ($mode){
			$q_str = "DELETE FROM smpl_wiqty WHERE wi_id='$id' ";
		}else{
			$q_str = "DELETE FROM smpl_wiqty WHERE id='$id' ";
		}
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function update_field($field, $val, $id) {
		...
		#####   更新資料庫內容
	$q_str = "UPDATE smpl_wiqty SET ".$field."='".$val."'  WHERE id=".$id." ";
