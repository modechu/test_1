...
class PDF_ti extends PDF_Chinese 	{
...
function Header() {
  ...
  $this->SetFont('Arial','I',9);
	...
	if ($ary_title['img_size'][0] > $ary_title['img_size'][1])
	{
		$this->Image($ary_title['img'],10,28,47,0);
	}else{
		$this->Image($ary_title['img'],10,28,0,47);
	}
	$this->ln();
	$Y = $this->getY();
	$X = $this->getX();
...
//creat ti's field
function ti_name_fields($name,$word='BIG5') {
//	$this->SetFillColor(177,176,154);
//	$this->SetTextColor(0);
	$this->setx(10);
    $this->SetLineWidth(.5);

    $this->SetFont($word,'B',10);
...
function ti_sub_name($name,$word='BIG5') {
//	$this->SetFillColor(177,176,154);
//	$this->SetTextColor(0);
	$this->setx(10);
  $this->SetLineWidth(.5);
	...
	$this->ln();
  $this->SetFont($word,'',14);
	$this->Cell(190,8,$name,'TLR',0,'C');
	$this->ln();
}

function ti_value_fields($value,$x,$item_no,$ti_item) {
	$this->SetTextColor(0);
//	$y=0;
    ...
    for ($i=0; $i<sizeof($tmp); $i++)
    {
			...
			$cut = 0;
				...
				for ($z=0; $z < $cut; $z++)
				{
					...
					{
						$this->setx(10);
						$this->Cell(190,5,'','LBR',0,'L');	
						$this->AddPage();		
						$this->SetFont('Arial','B',10);
						$this->ti_name_fields($item_no.")".$ti_item);
						$x=4;
					}					
					$tmp_ti = mb_substr($tmp[$i],$l1,52,'big-5'); 
...
function hend_title($smpl)
{
	global $stype;
	global $des_fab;
	
	$this->filed_1(70,28,35,15,7,$smpl['prt_ord'],'80,110,140',$smpl['id']);
	$this->filed_1(106,28,45,15,7,'Customer','80,110,140',$smpl['cust']);
	...
	$this->filed_1(112,45,48,15,7,'Size / Style','80,110,140',$smpl['size'].' /  '.$smpl['style']);
	$this->filed_1(161,45,15,15,7,'Q\'ty','80,110,140',$smpl['qty'].' '.$smpl['unit']);
	$this->filed_1(177,45,23,15,7,'ETD','80,110,140',$smpl['etd']);
...
//  $this->SetTextColor(0);	
  ...
  $this->setx(10); 
	...
	if ($acc['acc_name'] == 'other')
	{
		...
		$this->cell(20,5,$acc_name,'TBL',0,'L');
		$i=20;
...
//	if ($data['specify']) $x++;		
	...
	if ($data['weight'])
	{
		...
		$this->SetFont('Arial','B',8);	
		$this->cell(10,5,'width : ','TB',0,'C');
		$this->SetFont('Big5','',8);
		$this->cell(11,5,$data['width'],'TB',0,'L');
