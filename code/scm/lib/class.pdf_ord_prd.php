...
class PDF_ord_prd extends PDF_Chinese 	{
...
function Header() {
    ...
    $this->RotatedText(50,180,$mark,30);
	//Logo
...
//263
	...
	$this->Cell(21,6,'Order #','TLR',0,'C');
	$this->Cell(12,6,'sty','TLR',0,'C');
	$this->Cell(25,6,'ETD','TLR',0,'C'); 
	$this->Cell(50,6,'FAB',1,0,'C'); 
	$this->Cell(50,6,'ACC',1,0,'C');
	$this->Cell(25,6,'SMPL','TLR',0,'C');
	$this->Cell(25,6,'ETS','TLR',0,'C');	
	$this->Cell(25,6,'ETF','TLR',0,'C');
	$this->Cell(20,6,'Out-Put','TLR',0,'C');
	$this->Cell(15,6,'Lead','TLR',0,'C');	
	$this->Cell(13,6,'schd.','TLR',1,'C');


	$this->Cell(21,6,'','BLR',0,'C'); //order
	$this->Cell(12,6,'','BLR',0,'C'); //style
	$this->Cell(25,6,'','BLR',0,'C');  //etd
	$this->Cell(23,6,'ETA','BLR',0,'C'); //FAB ETA
	$this->Cell(27,6,'RCVD','BLR',0,'C'); //FAB RCVD
	$this->Cell(23,6,'ETA','BLR',0,'C'); //ACC ETA
	$this->Cell(27,6,'RCVD','BLR',0,'C'); //ACC RCVD
	$this->Cell(25,6,'APV','BLR',0,'C');//SMPL APV
	$this->Cell(25,6,'','BLR',0,'C');	//ETS
	$this->Cell(25,6,'','BLR',0,'C'); //ETF
	$this->Cell(20,6,'(PCs)','BLR',0,'C'); //out-pur (pcs)
	$this->Cell(15,6,'Time','BLR',0,'C'); // lead time
	$this->Cell(13,6,'','BLR',1,'C'); 
}


function ord_recs($parm)
{
	
	$mat_shp = $parm['mat_shp'];
	if ($parm['mat_rcv'] && $parm['mat_shp'] < '0000-00-00') $mat_shp = '('.substr($parm['mat_ship_way'],0,1).') '.$parm['mat_rcv'];
	if($parm['mat_shp'] == '1111-11-11') $mat_shp = '-------';

	$m_acc_shp = $parm['m_acc_shp'];
	if ($parm['m_acc_rcv'] && $parm['m_acc_shp'] < '0000-00-00') $m_acc_shp = '('.substr($parm['m_acc_ship_way'],0,1).') '.$parm['m_acc_rcv'];
	if($parm['m_acc_shp'] == '1111-11-11') $m_acc_shp = '-------';
/*
	$pdt_area = '';
	if($parm['in_house'] == 1)$pdt_area .='I';
	if($parm['sub_con'] == 1)$pdt_area .='S';
*/
		
	if(!isset($parm['sch_rec'][0]['line_name'])) $parm['sch_rec'][0]['line_name'] = '';
 	$this->SetFont('Arial','',9);
	$this->Cell(21,6,$parm['order_num'],'TLR',0,'C'); //order
	$this->Cell(12,6,$parm['style'],'TLR',0,'C'); //style
	$this->Cell(25,6,$parm['etd'],'TLR',0,'C');  //etd
	$this->Cell(23,6,$parm['mat_eta'],'TLR',0,'C'); //FAB ETA
	$this->Cell(27,6,$mat_shp,'TLR',0,'C'); //FAB RCVD
	$this->Cell(23,6,$parm['m_acc_eta'],'TLR',0,'C'); //ACC ETA
	$this->Cell(27,6,$m_acc_shp,'TLR',0,'C'); //ACC RCVD
	$this->Cell(25,6,$parm['smpl_apv'],'TLR',0,'C');//SMPL APV
	$this->Cell(25,6,$parm['ets'],'TLR',0,'C');	//ETS
	$this->Cell(25,6,$parm['etf'],'TLR',0,'C'); //ETF
	$this->Cell(20,6,$parm['qty_done'],'TLR',0,'C'); //out-pur (pcs)
	$this->Cell(15,6,$parm['lead'],'TLR',0,'C'); // lead time
	$this->Cell(13,6,$parm['sch_rec'][0]['line_name'],'TLR',1,'C'); 
	
	
	for($i=1; $i<sizeof($parm['sch_rec']); $i++)
	{
		$this->Cell(21,6,'','LR',0,'C'); //order
		$this->Cell(12,6,'','LR',0,'C'); //style
		$this->Cell(25,6,'','LR',0,'C');  //etd
		$this->Cell(23,6,'','LR',0,'C'); //FAB ETA
		$this->Cell(27,6,'','LR',0,'C'); //FAB RCVD
		$this->Cell(23,6,'','LR',0,'C'); //ACC ETA
		$this->Cell(27,6,'','LR',0,'C'); //ACC RCVD
		$this->Cell(25,6,'','LR',0,'C');//SMPL APV
		$this->Cell(25,6,'','LR',0,'C');	//ETS
		$this->Cell(25,6,'','LR',0,'C'); //ETF
		$this->Cell(20,6,'','LR',0,'C'); //out-pur (pcs)
		$this->Cell(15,6,'','LR',0,'C'); // lead time
		$this->Cell(13,6,$parm['sch_rec'][$i]['line_name'],'LR',1,'C'); 		
	}

	$this->Cell(21,0.3,'','BLR',0,'C'); //order
	$this->Cell(12,0.3,'','BLR',0,'C'); //style
	$this->Cell(25,0.3,'','BLR',0,'C');  //etd
	$this->Cell(23,0.3,'','BLR',0,'C'); //FAB ETA
	$this->Cell(27,0.3,'','BLR',0,'C'); //FAB RCVD
	$this->Cell(23,0.3,'','BLR',0,'C'); //ACC ETA
	$this->Cell(27,0.3,'','BLR',0,'C'); //ACC RCVD
	$this->Cell(25,0.3,'','BLR',0,'C');//SMPL APV
	$this->Cell(25,0.3,'','BLR',0,'C');	//ETS
	$this->Cell(25,0.3,'','BLR',0,'C'); //ETF
	$this->Cell(20,0.3,'','BLR',0,'C'); //out-pur (pcs)
	$this->Cell(15,0.3,'','BLR',0,'C'); // lead time
	$this->Cell(13,0.3,'','BLR',1,'C'); 	
}

function show_mat_recs($parm)
{
	$this->SetFont('Big5','',8);
	for($i=0; $i<sizeof($parm); $i++)
	{	 		
		$this->Cell(281,6,$parm[$i],1,1,'L');
	}
...
function filed_1($x,$y,$w,$h,$h2,$title,$color,$field,$alin='C') {
	...
	if ($title == 'Dept.' || $title == 'Supplier Name'){$this->SetFont('big5','IB',12);}else{$this->SetFont('Arial','IB',12);}
