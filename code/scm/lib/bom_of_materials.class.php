...
while ($partial = $sql->fetch($q_result)) {
    ...
    while ($colorway = $sql->fetch($q_results)) {
        ...
        $color_size[0][$colorway['wi_id']][$id]['qty'] = array_sum($color_pdc_array);
        $color_size[0][$colorway['wi_id']][$id]['color'] = $colorway['pdc_color'];
        for( $i=0; $i < count($size_range); $i++ ){
            @$color_size[1][$colorway['wi_id']][$size_range[$i]]['p_id'] = $partial['id'];
            @$color_size[1][$colorway['wi_id']][$size_range[$i]]['qty'] += $color_pdc_array[$i];
            @$color_size[1][$colorway['wi_id']][$size_range[$i]]['color'] = $colorway['pdc_color'];
            @$color_size[2][$colorway['wi_id']][$size_range[$i]][$id]['p_id'] = $partial['id'];
            @$color_size[2][$colorway['wi_id']][$size_range[$i]][$id]['qty'] += $color_pdc_array[$i];
            @$color_size[2][$colorway['wi_id']][$size_range[$i]][$id]['color'] = $colorway['pdc_color'];
        }
        $color_size[3][$colorway['wi_id']]['p_id'] = $partial['id'];
        $color_size[3][$colorway['wi_id']]['qty'] = array_sum($color_pdc_array);
        $color_size[3][$colorway['wi_id']]['color'] = $colorway['pdc_color'];
    }
...
#
# :GET_WIQTY_QTY_SEND
function get_wiqty_qty_send($wi_num,$size_arr,$send_qty) {
...
$color_size = array();
$partial = array();
$size_g_ttl = $size_pdc_g_ttl = $size_requi_g_ttl = array();
...
while ($partial = $sql->fetch($q_result)) {
    ...
    $colorway = array();
    $q_results = $sql->query($q_strs);
    $size_sum_ttl = $size_pdc_sum_ttl = $size_requi_sum_ttl = array();
    ...
    while ($colorway = $sql->fetch($q_results)) {
        ...
        $CHTML .= '<td rowspan="2" id="Bom_TR_Item" colspan="3" bgcolor="EEEEEE">'.$colorway['pdc_color'].'</td>';
        ...
        $color_array = $color_pdc_array = $color_requi_array = array();
        ...
        $c_requi_row = explode(",",$send_qty[$id]);
        foreach( $c_requi_row as $cVal){
            $color_requi_array[] = $cVal;
        }
        
        $c_pdc_row = explode(",",$colorway['pdc_qty']);
        foreach( $c_pdc_row as $k => $cVal){
            $color_pdc_array[] = $cVal - $color_requi_array[$k];
        }
        
        $color_sum = $color_pdc_sum = $color_requi_sum = 0;
        foreach( $size_range as $skey => $v){
        $base_size_str = $base_size == $v ? 'style="color:#0fb562;"' : '' ;
        $CHTML .= '<td id="Bom_TR_Value" x="PHP_qty_'.$id.'_'.$v.'" '.$base_size_str.'>'.number_format($color_pdc_array[$skey],0,'',',').'</td>';
        $CHTML_PDT .= '<td id="Bom_TR_Value2" '.$base_size_str.'>
        <input class="input_bom_qty" xname="PHP_qty_'.$id.'_'.$v.'" yname="PHP_qty_'.$p_id.'_'.$v.'" name="PHP_qty['.$id.']['.$v.']" value="" o_qty="'.$color_pdc_array[$skey].'" onkeyup="cal_send_material(this,\''.$id.'\',\''.$v.'\',\''.$p_id.'\')" '.($color_array[$skey]>0?'':'readonly').'>
        </td>';
        ...
        $color_pdc_sum += $color_pdc_array[$skey];
        $color_requi_sum += $color_requi_array[$skey];
        $size_sum_ttl[$skey] += $color_array[$skey];
        $size_pdc_sum_ttl[$skey] += $color_pdc_array[$skey];
        $size_requi_sum_ttl[$skey] += $color_requi_array[$skey];
        $size_g_ttl[$skey] += $color_array[$skey];
        $size_pdc_g_ttl[$skey] += $color_pdc_array[$skey];
        $size_requi_g_ttl[$skey] += $color_requi_array[$skey];
        }
        $CHTML .= '<td id="Bom_TR_Total" oname="PHP_sum_'.$id.'" bgcolor="EEEEEE">'.number_format($color_pdc_sum,0,'',',').'</td>';
        $CHTML_PDT .= '<td id="Bom_TR_Total2" xname="PHP_sum_'.$id.'" bgcolor="EEEEEE"></td>';
        $CHTML .= '</tr>';

        $CHTML .= '<tr mark="1" class="Bom_dis_'.$partial['id'].'" id="Bom_TR_hover">'.$CHTML_PDT.'</tr>';
    
        $DHTML .= '<td id="Bom_TR_Total" bgcolor="EEEEEE">'.number_format($color_pdc_sum,0,'',',').'</td>';
        $DHTML_PDT .= '<td id="Bom_TR_Total2" xname="PHP_sum_'.$id.'" bgcolor="EEEEEE">'.number_format($color_requi_sum,0,'',',').'</td>';
        $DHTML .= '</tr>';
    ...
    $color_sum_ttl = 0;
    foreach( $size_pdc_sum_ttl as $tkey => $tval ){
        $CHTML .= '<td id="Bom_TR_Total" xsname="PHP_sum_'.$partial['id'].'_'.$size_range[$tkey].'" bgcolor="CCCCCC">'.number_format($tval,0,'',',').'</td>';
        $DHTML .= '<td id="Bom_TR_Total" bgcolor="CCCCCC"></td>';
        $color_pdc_sum_ttl += $tval;
    }
    $CHTML .= '<td id="Bom_TR_Total" xysname="PHP_sum_'.$partial['id'].'" bgcolor="CCCCCC">'.number_format($color_pdc_sum_ttl,0,'',',').'</td>';
    $CHTML .= '</tr>';
    $DHTML .= '<td id="Bom_TR_Total" bgcolor="CCCCCC">'.number_format($color_pdc_sum_ttl,0,'',',').'</td>';
    $DHTML .= '</tr>';
    
    $CHTML .= '<tr class="Bom_dis_'.$partial['id'].'" id="Bom_TR_hover">';
    $CHTML .= '<td id="Bom_TR_Total" bgcolor="CCCCCC" colspan="3">SEND TOTAL</td>';
    $DHTML .= '<tr class="Bom_dis_'.$partial['id'].'" id="Bom_TR_hover">';
    $DHTML .= '<td id="Bom_TR_Total" bgcolor="CCCCCC" colspan="3">SEND TOTAL</td>';
    $color_requi_sum_ttl = 0;
    foreach( $size_requi_sum_ttl as $tkey => $tval ){
        $CHTML .= '<td id="Bom_TR_Total" xxname="PHP_sum_'.$size_range[$tkey].'" yname="PHP_sum_'.$partial['id'].'_'.$size_range[$tkey].'" bgcolor="CCCCCC"></td>';
        ...
        $color_requi_sum_ttl += $tval;
    }
    $CHTML .= '<td id="Bom_TR_Total" xyname="PHP_sum_'.$partial['id'].'" bgcolor="CCCCCC"></td>';
    $CHTML .= '</tr>';

    $DHTML .= '<td id="Bom_TR_Total" xyname="PHP_sum_'.$partial['id'].'" bgcolor="CCCCCC">'.number_format($color_requi_sum_ttl,0,'',',').'</td>';
    $DHTML .= '</tr>';
    ...
    $color_pdc_g_ttl = 0;
    foreach( $size_pdc_g_ttl as $tkey => $tval ){
        $CHTML .= '<td id="Bom_TR_GTotal" o="PHP_gtotal_'.$size_range[$tkey].'" bgcolor="ffffbf">'.number_format($tval,0,'',',').'</td>';
        $DHTML .= '<td id="Bom_TR_GTotal" bgcolor="ffffbf">'.number_format($tval,0,'',',').'</td>';
        $color_pdc_g_ttl += $tval;
    }
    $CHTML .= '<td id="Bom_TR_GTotal" o="PHP_gtotal" bgcolor="ffffbf">'.number_format($color_pdc_g_ttl,0,'',',').'</td>';
    $CHTML .= '</tr>';
    $DHTML .= '<td id="Bom_TR_GTotal" bgcolor="ffffbf">'.number_format($color_pdc_g_ttl,0,'',',').'</td>';
    $DHTML .= '</tr>';
    
    $CHTML .= '<tr class="Bom_dis_dttl" id="Bom_TR_hover">';
    $CHTML .= '<td id="Bom_TR_GTotal" bgcolor="ffffbf" colspan="3" algan="right">SEND G-TOTAL</td>';
    $DHTML .= '<tr class="Bom_dis_dttl" id="Bom_TR_hover">';
    $DHTML .= '<td id="Bom_TR_GTotal" bgcolor="ffffbf" colspan="3" algan="right">SEND G-TOTAL</td>';
    $color_requi_g_ttl = 0;
    foreach( $size_requi_g_ttl as $tkey => $tval ){
        $CHTML .= '<td id="Bom_TR_GTotal" yname="PHP_gtotal_'.$size_range[$tkey].'" bgcolor="ffffbf"></td>';
        $DHTML .= '<td id="Bom_TR_GTotal" yname="PHP_gtotal_'.$size_range[$tkey].'" bgcolor="ffffbf">'.number_format($tval,0,'',',').'</td>';
        $color_requi_g_ttl += $tval;
    }
    $CHTML .= '<td id="Bom_TR_GTotal" xyname="PHP_gtotal" bgcolor="ffffbf"></td>';
    $CHTML .= '</tr>';
    $DHTML .= '<td id="Bom_TR_GTotal" xyname="PHP_gtotal" bgcolor="ffffbf">'.number_format($color_requi_g_ttl,0,'',',').'</td>';
    $DHTML .= '</tr>';
...
#
# :GET_WIQTY_QTY_SEND
function get_wiqty_qty_send_edit($wi_num,$size_arr) {
...
while ($partial = $sql->fetch($q_result)) {
    ...
    while ($colorway = $sql->fetch($q_results)) {
        ...
        $c_requi_row = explode(",",$colorway['pdc_qty']);
        foreach( $c_requi_row as $cVal){
            $color_requi_array[] = $cVal;
        }
        ...
        foreach( $size_range as $skey => $v){
        $base_size_str = $base_size == $v ? 'style="color:#0fb562;"' : '' ;
        $CHTML .= '<td id="Bom_TR_Value" '.$base_size_str.'>'.number_format($color_pdc_array[$skey],0,'',',').'</td>';
        $CHTML_PDT .= '<td id="Bom_TR_Value2" '.$base_size_str.'><input class="input_bom_qty" xname="PHP_qty_'.$id.'_'.$v.'" yname="PHP_qty_'.$p_id.'_'.$v.'" name="PHP_qty['.$id.']['.$v.']" value="'.$color_requi_array[$skey].'" onkeyup="cal_fty_colorway(this,\''.$id.'\',\''.$v.'\',\''.$p_id.'\')" '.($color_array[$skey]>0?'':'readonly').'></td>';
        ...
        $CHTML .= '<td id="Bom_TR_Total" bgcolor="EEEEEE">'.number_format($color_pdc_sum,0,'',',').'</td>';
    ...
    $CHTML .= '<td id="Bom_TR_Total" bgcolor="CCCCCC">'.number_format($color_pdc_sum_ttl,0,'',',').'</td>';
    $CHTML .= '</tr>';
    $DHTML .= '<td id="Bom_TR_Total" bgcolor="CCCCCC">'.number_format($color_pdc_sum_ttl,0,'',',').'</td>';
    $DHTML .= '</tr>';
    ...
    $CHTML .= '<td id="Bom_TR_Total" xyname="PHP_sum_'.$partial['id'].'" bgcolor="CCCCCC">'.number_format($color_requi_sum_ttl,0,'',',').'</td>';
    $CHTML .= '</tr>';
    ...
    $CHTML .= '<td id="Bom_TR_GTotal" bgcolor="ffffbf">'.number_format($color_pdc_g_ttl,0,'',',').'</td>';
    $CHTML .= '</tr>';
    $DHTML .= '<td id="Bom_TR_GTotal" bgcolor="ffffbf">'.number_format($color_pdc_g_ttl,0,'',',').'</td>';
    $DHTML .= '</tr>';
    ...
    $CHTML .= '<td id="Bom_TR_GTotal" xyname="PHP_gtotal" bgcolor="ffffbf">'.number_format($color_requi_g_ttl,0,'',',').'</td>';
    $CHTML .= '</tr>';
    $DHTML .= '<td id="Bom_TR_GTotal" xyname="PHP_gtotal" bgcolor="ffffbf">'.number_format($color_requi_g_ttl,0,'',',').'</td>';
    $DHTML .= '</tr>';
...
#
#
# :GET_WIQTY_QTY_SEND_MATERIAL
function get_wiqty_qty_send_material($wi_num,$size_arr,$utf_bom_qty) {
...
while ($partial = $sql->fetch($q_result)) {
    ...
    while ($colorway = $sql->fetch($q_results)) {
        ...
        $CHTML_PDT = '';
        $CHTML_PDT .= '<tr mark="1" class="Bom_dis_'.$partial['id'].'" id="Bom_TR_hover">';
        $CHTML_PDT .= '<td id="Bom_TR_Item" colspan="3" bgcolor="EEEEEE">'.$colorway['pdc_color'].'</td>';
        // $CHTML_PDT .= '<td id="Bom_TR_Item2" colspan="3" bgcolor="EEEEEE"><input class="input_bom_txt" name="PHP_color['.$id.']" value="'.$colorway['pdc_color'].'"></td>';
        ...
        $c_requi_row = explode(",",$utf_bom_qty[$id]);
        foreach( $c_requi_row as $cVal){
            $color_requi_array[] = $cVal;
        }
        ...
        $base_size_str = $base_size == $v ? 'style="color:#0fb562;"' : '' ;
        // $CHTML .= '<td id="Bom_TR_Value" '.$base_size_str.'>'.number_format($color_pdc_array[$skey],0,'',',').'</td>';
        $CHTML_PDT .= '<td id="Bom_TR_Value" '.$base_size_str.'>'.number_format($color_requi_array[$skey],0,'',',').'</td>';
        
        // $DHTML .= '<td id="Bom_TR_Value" '.$base_size_str.'>'.number_format($color_array[$skey],0,'',',').'</td>';
        $DHTML_PDT .= '<td id="Bom_TR_Value" '.$base_size_str.'>'.$color_pdc_array[$skey].'</td>';
        ...
        // $CHTML .= '<td id="Bom_TR_Total" bgcolor="EEEEEE">'.number_format($color_pdc_sum,0,'',',').'</td>';
        $CHTML_PDT .= '<td id="Bom_TR_Total" xname="PHP_sum_'.$id.'" bgcolor="EEEEEE">'.number_format($color_requi_sum,0,'',',').'</td>';
        // $CHTML .= '</tr>';

        $CHTML .= '<tr mark="1" class="Bom_dis_'.$partial['id'].'" id="Bom_TR_hover">'.$CHTML_PDT.'</tr>';
    
        // $DHTML .= '<td id="Bom_TR_Total" bgcolor="EEEEEE">'.number_format($color_pdc_sum,0,'',',').'</td>';
        $DHTML_PDT .= '<td id="Bom_TR_Total" xname="PHP_sum_'.$id.'" bgcolor="EEEEEE">'.number_format($color_pdc_sum,0,'',',').'</td>';
        // $DHTML .= '</tr>';
        ...
        # ARRAY
        $color_size[0][$colorway['wi_id']][$id]['p_id'] = $partial['id'];
        $color_size[0][$colorway['wi_id']][$id]['qty'] = array_sum($color_requi_array);
        $color_size[0][$colorway['wi_id']][$id]['colorway'] = $colorway['colorway'];
        for( $i=0; $i < count($size_range); $i++ ){
            @$color_size[1][$colorway['wi_id']][$size_range[$i]]['p_id'] = $partial['id'];
            @$color_size[1][$colorway['wi_id']][$size_range[$i]]['qty'] += $color_requi_array[$i];
            @$color_size[1][$colorway['wi_id']][$size_range[$i]]['colorway'] = $colorway['colorway'];
            @$color_size[2][$colorway['wi_id']][$size_range[$i]][$id]['p_id'] = $partial['id'];
            @$color_size[2][$colorway['wi_id']][$size_range[$i]][$id]['qty'] += $color_requi_array[$i];
            @$color_size[2][$colorway['wi_id']][$size_range[$i]][$id]['colorway'] = $colorway['colorway'];
        }
        $color_size[3][$colorway['wi_id']]['p_id'] = $partial['id'];
        $color_size[3][$colorway['wi_id']]['qty'] += array_sum($color_requi_array);
        $color_size[3][$colorway['wi_id']]['colorway'] = $colorway['colorway'];
    }
...
return array($PHTML,$CHTML,$color_size,$DHTML,number_format($color_requi_g_ttl,0,'',','));
...
#
#
# :GET_WIQTY_QTY_VIEW_MATERIAL
function get_wiqty_qty_view_material($wi_num,$size_arr,$utf_bom_qty) {
...
while ($partial = $sql->fetch($q_result)) {
    ...
    $colorway = array();
    $q_results = $sql->query($q_strs);
    $colorway_arr = $size_sum_ttl = $size_pdc_sum_ttl = $size_requi_sum_ttl = array();
    ...
    while ($colorway = $sql->fetch($q_results)) {
        ...
        $colorway['qty'];
        $colorway['pdc_color'];
        $colorway['pdc_qty'] = $utf_bom_qty[$colorway['id']];
        $colorway['item'];
        $colorway['nnw'];
        $colorway['ship_color'];
        $colorway['p_id'];
        if( !$colorway_arr[$colorway['p_id'].$colorway['pdc_color']] ){
            ...
            implode( ',' , $this->adding( $colorway_arr[$colorway['p_id'].$colorway['pdc_color']]['pdc_qty'] , $utf_bom_qty[$colorway['id']] ) );
        }
    }
    
    // print_r($colorway_arr);
...
return array($PHTML,$CHTML,$color_size,$DHTML,number_format($color_pdc_g_ttl,0,'',','));
...
#
#
# :GET_WIQTY_QTY_EDIT_MATERIAL
function get_wiqty_qty_edit_material($wi_num,$size_arr,$send_qty,$utf_bom_qty) {
...
$color_size = array();
$partial = array();
$size_g_ttl = $size_pdc_g_ttl = $size_requi_g_ttl = $size_send_g_ttl = array();
...
while ($partial = $sql->fetch($q_result)) {
    ...
    $colorway = array();
    $q_results = $sql->query($q_strs);
    $size_sum_ttl = $size_pdc_sum_ttl = $size_send_sum_ttl = $size_requi_sum_ttl = array();
    ...
    while ($colorway = $sql->fetch($q_results)) {
        ...
        // $DHTML .= '<td id="Bom_TR_Item" colspan="3" bgcolor="EEEEEE">'.$colorway['colorway'].'</td>';
        // $DHTML_PDT .= '<td id="Bom_TR_Item2" colspan="3">'.$colorway['pdc_color'].'</td>';
        $color_array = $color_pdc_array = $color_send_array = $color_requi_array = array();
        ...
        $c_send_row = explode(",",$send_qty[$id]);
        foreach( $c_send_row as $cVal){
            $color_send_array[] = $cVal;
        }
        ...
        foreach( $size_range as $skey => $v){
        $base_size_str = $base_size == $v ? 'style="color:#0fb562;"' : '' ;
        $CHTML .= '<td id="Bom_TR_Value" x="PHP_qty_'.$id.'_'.$v.'" '.$base_size_str.'>'.number_format(($color_pdc_array[$skey]-$color_send_array[$skey]-$color_requi_array[$skey]),0,'',',').'</td>';
        $CHTML_PDT .= '<td id="Bom_TR_Value2" '.$base_size_str.'>
        <input class="input_bom_qty" xname="PHP_qty_'.$id.'_'.$v.'" yname="PHP_qty_'.$p_id.'_'.$v.'" name="PHP_qty['.$id.']['.$v.']" value="'.$color_requi_array[$skey].'" o_qty="'.($color_pdc_array[$skey]-$color_send_array[$skey]).'" onkeyup="cal_send_material(this,\''.$id.'\',\''.$v.'\',\''.$p_id.'\')" '.($color_array[$skey]>0?'':'readonly').'>
        </td>';
        
        // $DHTML .= '<td id="Bom_TR_Value" '.$base_size_str.'>'.number_format($color_array[$skey],0,'',',').'</td>';
        // $DHTML_PDT .= '<td id="Bom_TR_Value2" '.$base_size_str.'>'.$color_pdc_array[$skey].'</td>';
        // echo $color_pdc_array[$skey].'-'.$color_send_array[$skey].'-'.$color_requi_array[$skey].'='.($color_pdc_array[$skey]-$color_send_array[$skey]-$color_requi_array[$skey]).'<br>';
        $color_sum += $color_array[$skey];
        $color_pdc_sum += ($color_pdc_array[$skey]-$color_send_array[$skey]-$color_requi_array[$skey]);
        $color_requi_sum += $color_requi_array[$skey];
        $color_send_sum += $color_send_array[$skey];
        $size_sum_ttl[$skey] += $color_array[$skey];
        $size_pdc_sum_ttl[$skey] += ($color_pdc_array[$skey]-$color_send_array[$skey]-$color_requi_array[$skey]);
        $size_requi_sum_ttl[$skey] += $color_requi_array[$skey];
        $size_send_sum_ttl[$skey] += $color_send_array[$skey];
        $size_g_ttl[$skey] += $color_array[$skey];
        $size_pdc_g_ttl[$skey] += ($color_pdc_array[$skey]-$color_send_array[$skey]-$color_requi_array[$skey]);
        $size_requi_g_ttl[$skey] += $color_requi_array[$skey];
        $size_send_g_ttl[$skey] += $color_send_array[$skey];
        }
        $CHTML .= '<td id="Bom_TR_Total" oname="PHP_sum_'.$id.'" bgcolor="EEEEEE">'.number_format($color_pdc_sum,0,'',',').'</td>';
        $CHTML_PDT .= '<td id="Bom_TR_Total2" xname="PHP_sum_'.$id.'" bgcolor="EEEEEE">'.number_format($color_requi_sum,0,'',',').'</td>';
        $CHTML .= '</tr>';
...
#
# ->GET_BOM_VIEW()
function get_bom_view($wi_id,$size_arr,$color_size,$bom_utf8) {
// print_r($size_arr);
// print_r($color_size);
$sql = $this->sql;
...
# Fab
$q_str = "SELECT 
`bom_lots`.`id`,`bom_lots`.`wi_id`,`bom_lots`.`lots_used_id`,`bom_lots`.`color`,`bom_lots`.`qty`,`bom_lots`.`o_qty`,`bom_lots`.`qty`,`bom_lots`.`ap_mark`,
`bom_lots`.`size`,`bom_lots`.`pp_mark`,`bom_lots`.`dis_ver`,
`lots_use`.`lots_code`,`lots_use`.`lots_name`,`lots_use`.`unit`,`lots_use`.`est_1`,`lots_use`.`use_for`,`lots_use`.`support` , 
`lots`.`id` as `lots_id`, `lots`.`des` , `lots`.`comp` , `lots`.`memo` , `lots`.`width` 
...
// echo $q_str.'<br>'; #,`bom_lots`.`pdc_color`,`bom_lots`.`pdc_qty`
if (!$q_result = $sql->query($q_str)) {
	$this->msg->merge($srh->msg);
	return false;
}
...
$TR_NUM = 0;
foreach($bom_lots as $mat){

    $bom_id = $mat['id'];

    if( $TR_NUM % 15 == 0 ){
    $BHTML .= '<tr id="Bom_TR_Lots_Title" style="height:36px;">';
    $BHTML .= '<td>Fabric #</td>';
    $BHTML .= '<td>Fabric Name</td>';
    $BHTML .= '<td>BOM. Color</td>';
    $BHTML .= '<td>BOM. YY.</td>';
    $BHTML .= '<td>Size</td>';
    $BHTML .= '<td>Loss%</td>';
    $BHTML .= '<td>BOM Require Qty</td>';
    $BHTML .= '</tr>';
    }
    $TR_NUM++;
    
    $utf8_id = $bom_utf8['utf8']['l'][$mat['id']]['id']?$bom_utf8['utf8']['l'][$mat['id']]['id']:'';
    $utf8_name = $bom_utf8['utf8']['l'][$mat['id']]['mat_name']?$bom_utf8['utf8']['l'][$mat['id']]['mat_name']:'';
    $utf8_mat_code = $bom_utf8['utf8']['l'][$mat['id']]['mat_code']?$bom_utf8['utf8']['l'][$mat['id']]['mat_code']:'';
    $utf8_color = $bom_utf8['utf8']['l'][$mat['id']]['color']?$bom_utf8['utf8']['l'][$mat['id']]['color']:'';
    $utf8_o_color = $bom_utf8['utf8']['l'][$mat['id']]['o_color']?$bom_utf8['utf8']['l'][$mat['id']]['o_color']:'';
    $utf8_size = $bom_utf8['utf8']['l'][$mat['id']]['size']?$bom_utf8['utf8']['l'][$mat['id']]['size']:'';
    $utf8_unit = $bom_utf8['utf8']['l'][$mat['id']]['unit']?$bom_utf8['utf8']['l'][$mat['id']]['unit']:'';
    $utf8_qty = $bom_utf8['utf8']['l'][$mat['id']]['qty']?$bom_utf8['utf8']['l'][$mat['id']]['qty']:0;
    $utf8_o_qty = $bom_utf8['utf8']['l'][$mat['id']]['o_qty']?$bom_utf8['utf8']['l'][$mat['id']]['o_qty']:0;
    $utf8_loss = $bom_utf8['utf8']['l'][$mat['id']]['loss']?$bom_utf8['utf8']['l'][$mat['id']]['loss']:0;
    $utf8_consump = $bom_utf8['utf8']['l'][$mat['id']]['consump']?$bom_utf8['utf8']['l'][$mat['id']]['consump']:0;

    $BHTML .= '<tr id="Bom_TR_List">';
    $BHTML .= '<td><a href="javascript:Show_fab(\''.$utf8_mat_code.'\')" style="cursor:pointer">'.$utf8_mat_code.'</a></td>';
    $BHTML .= '<td align="left">'.$utf8_name.'</td>';
    $BHTML .= '<td align="left">'.$utf8_color.'</td>';
    $BHTML .= '<td align="right">'.$utf8_consump.' '.$utf8_unit.'</td>';
    $BHTML .= '<td>'.$utf8_size.'</td>';
    $BHTML .= '<td align="right">'.$utf8_loss.'</td>';
    $BHTML .= '<td align="right">'.number_format($utf8_qty,0,'',',').' '.$utf8_unit.'</td>';
}

# Acc
$q_str = "SELECT 
`bom_acc`.`id`,`bom_acc`.`wi_id`,`bom_acc`.`acc_used_id`,`bom_acc`.`color`,`bom_acc`.`qty`,`bom_acc`.`o_qty`,`bom_acc`.`ap_mark`,`bom_acc`.`size`,`bom_acc`.`pp_mark`,`bom_acc`.`dis_ver`,
`acc_use`.`acc_code`,`acc_use`.`acc_name`,`acc_use`.`unit`,`acc_use`.`est_1`,`acc_use`.`use_for`,`acc_use`.`support` , 
`acc`.`id` as `acc_id`,`acc`.`des` , `acc`.`specify` , `acc`.`specify` , `acc`.`memo` , `acc`.`mile_name` , `acc`.`arrange` 
...
$TR_NUM = 0;
foreach($bom_acc as $mat){

    $bom_id = $mat['id'];

    if( $TR_NUM % 15 == 0 ){
    $BHTML .= '<tr id="Bom_TR_Lots_Title" style="height:36px;">';
    $BHTML .= '<td>Accessory #</td>';
    $BHTML .= '<td>Accessory Name</td>';
    $BHTML .= '<td>BOM. Color</td>';
    $BHTML .= '<td>BOM. YY.</td>';
    $BHTML .= '<td>Size</td>';
    $BHTML .= '<td>Loss%</td>';
    ...
    $utf8_id = $bom_utf8['utf8']['a'][$mat['id']]['id']?$bom_utf8['utf8']['a'][$mat['id']]['id']:'';
    $utf8_name = $bom_utf8['utf8']['a'][$mat['id']]['mat_name']?$bom_utf8['utf8']['a'][$mat['id']]['mat_name']:'';
    $utf8_mat_code = $bom_utf8['utf8']['a'][$mat['id']]['mat_code']?$bom_utf8['utf8']['a'][$mat['id']]['mat_code']:'';
    $utf8_color = $bom_utf8['utf8']['a'][$mat['id']]['color']?$bom_utf8['utf8']['a'][$mat['id']]['color']:'';
    $utf8_o_color = $bom_utf8['utf8']['a'][$mat['id']]['o_color']?$bom_utf8['utf8']['a'][$mat['id']]['o_color']:'';
    $utf8_size = $bom_utf8['utf8']['a'][$mat['id']]['size']?$bom_utf8['utf8']['a'][$mat['id']]['size']:'';
    $utf8_unit = $bom_utf8['utf8']['a'][$mat['id']]['unit']?$bom_utf8['utf8']['a'][$mat['id']]['unit']:'';
    $utf8_qty = $bom_utf8['utf8']['a'][$mat['id']]['qty']?$bom_utf8['utf8']['a'][$mat['id']]['qty']:0;
    $utf8_o_qty = $bom_utf8['utf8']['a'][$mat['id']]['o_qty']?$bom_utf8['utf8']['a'][$mat['id']]['o_qty']:0;
    $utf8_loss = $bom_utf8['utf8']['a'][$mat['id']]['loss']?$bom_utf8['utf8']['a'][$mat['id']]['loss']:0;
    $utf8_consump = $bom_utf8['utf8']['a'][$mat['id']]['consump']?$bom_utf8['utf8']['a'][$mat['id']]['consump']:0;

    $BHTML .= '<tr id="Bom_TR_List">';
    $BHTML .= '<td><a href="javascript:Show_acc(\''.$utf8_mat_code.'\')" style="cursor:pointer">'.$utf8_mat_code.'</a></td>';
    $BHTML .= '<td align="left">'.$utf8_name.'</td>';
    $BHTML .= '<td align="left">'.$utf8_color.'</td>';
    $BHTML .= '<td align="right">'.$utf8_consump.' '.$utf8_unit.'</td>';
    $BHTML .= '<td>'.$utf8_size.'</td>';
...
#
# ->GET_BOM_DETAIL()
function get_bom_detail($wi_id,$size_arr,$color_size,$bom_utf8) {
// print_r($size_arr);
// print_r($color_size);
$sql = $this->sql;
...
foreach($bom_lots as $mat){
    ...
    $BHTML .= '<tr id="Bom_TR_Lots_Title" style="height:36px;">';
    $BHTML .= '<td>Fabric #</td>';
    $BHTML .= '<td colspan="2">Fabric Name</td>';
    $BHTML .= '<td colspan="2">BOM. Color</td>';
    $BHTML .= '<td colspan="2">BOM. YY.</td>';
    $BHTML .= '<td>Size</td>';
    $BHTML .= '<td>Loss%</td>';
    $BHTML .= '<td colspan="2">BOM Require Qty</td>';
    $BHTML .= '</tr>';
    }
    $TR_NUM++;
    ...
    $BHTML .= '<tr id="Bom_TR_List">';
    $BHTML .= '<td><a href="javascript:Show_fab(\''.$utf8_mat_code.'\')" style="cursor:pointer">'.$utf8_mat_code.'</a></td>';
    $BHTML .= '<td bgcolor="#ffffed" align="left">'.$mat['lots_name'].'</td>';
    $BHTML .= '<td align="left">'.$utf8_name.'</td>';
    $BHTML .= '<td bgcolor="#ffffed" align="left">'.big52uni($mat['color']).'</td>';
    $BHTML .= '<td align="left">'.$utf8_color.'</td>';
    $BHTML .= '<td bgcolor="#ffffed" align="right">'.$mat['est_1'].' '.$utf8_unit.'</td>';
    $BHTML .= '<td align="right">'.$utf8_consump.' '.$utf8_unit.'</td>';
    $BHTML .= '<td>'.$utf8_size.'</td>';
    $BHTML .= '<td align="right">'.$utf8_loss.'</td>';
    $BHTML .= '<td bgcolor="#ffffed" align="right">'.number_format($utf8_o_qty,0,'',',').' '.$utf8_unit.'</td>';
    $BHTML .= '<td align="right">'.number_format($utf8_qty,0,'',',').' '.$utf8_unit.'</td>';
}
...
foreach($bom_acc as $mat){
    ...
    $BHTML .= '<tr id="Bom_TR_Lots_Title" style="height:36px;">';
    $BHTML .= '<td>Accessory #</td>';
    $BHTML .= '<td colspan="2">Accessory Name</td>';
    $BHTML .= '<td colspan="2">BOM. Color</td>';
    $BHTML .= '<td colspan="2">BOM. YY.</td>';
    $BHTML .= '<td>Size</td>';
    $BHTML .= '<td>Loss%</td>';
    ...
    $BHTML .= '<tr id="Bom_TR_List">';
    $BHTML .= '<td><a href="javascript:Show_acc(\''.$utf8_mat_code.'\')" style="cursor:pointer">'.$utf8_mat_code.'</a></td>';
    $BHTML .= '<td bgcolor="#ffffed" align="left">'.$mat['acc_name'].'</td>';
    $BHTML .= '<td align="left">'.$utf8_name.'</td>';
    $BHTML .= '<td bgcolor="#ffffed" align="left">'.big52uni($mat['color']).'</td>';
    $BHTML .= '<td align="left">'.$utf8_color.'</td>';
    $BHTML .= '<td bgcolor="#ffffed" align="right">'.$mat['est_1'].' '.$utf8_unit.'</td>';
...
#
# ->GET_BOM_EDIT()
function get_bom_edit($wi_id,$size_arr,$color_size,$bom_utf8) {
// print_r($size_arr);
// print_r($bom_utf8);
$sql = $this->sql;
...
foreach($bom_lots as $mat){
    ...
    $BHTML .= '<tr id="Bom_TR_List">';
    $BHTML .= '<td><a href="javascript:Show_fab(\''.$mat['lots_code'].'\')" style="cursor:pointer">'.$mat['lots_code'].'</a></td>';
    $BHTML .= '<input type="hidden" name="PHP_lots_code['.$bom_id.']" value="'.($mat['lots_code']).'">';
    $BHTML .= '<input type="hidden" name="PHP_lots_id['.$bom_id.']" value="'.($mat['lots_id']).'">';
    $BHTML .= '<td align="left" bgcolor="#ffffed">'.$mat['lots_name'].'</td>';
    $BHTML .= '<td><input type="text" name="PHP_lots_name['.$bom_id.']" style="width:120px" class="styled-input" value="'.$utf8_name.'"></td>';
    $BHTML .= '<td bgcolor="#ffffed" align="left">'.($utf8_o_color?$utf8_o_color:big52uni($mat['color'])).'</td>';
    $BHTML .= '<input type="hidden" name="PHP_lots_o_color['.$bom_id.']" value="'.big52uni($mat['color']).'">';
    $BHTML .= '<td><input type="text" name="PHP_lots_color['.$bom_id.']" style="width:120px" class="styled-input" value="'.($utf8_color?$utf8_color:big52uni($mat['color'])).'"></td>';
    $BHTML .= '<td align="right" bgcolor="#ffffed">'.number_format($mat['est_1'],3,'',',').' '.$mat['unit'].'</td>';
    $BHTML .= '<td><input type="text" onclick="cell_loss(\'lots\',\''.$bom_id.'\')" onkeyup="cell_loss(\'lots\',\''.$bom_id.'\')" id="PHP_lots_consump_'.$bom_id.'" name="PHP_lots_consump['.$bom_id.']" class="styled-input" style="width:40px" value="'.($utf8_consump>0?$utf8_consump:$mat['est_1']).'"> '.$mat['unit'].'</td>';
    $BHTML .= '<td>'.$mat['size'].'</td>';
    $BHTML .= '<input type="hidden" name="PHP_lots_size['.$bom_id.']" value="'.($mat['size']).'">';
    $BHTML .= '<td><input type="text" onclick="cell_loss(\'lots\',\''.$bom_id.'\')" onkeyup="cell_loss(\'lots\',\''.$bom_id.'\')" id="PHP_lots_loss_'.$bom_id.'" name="PHP_lots_loss['.$bom_id.']" style="width:30px" class="styled-input" value="'.($utf8_loss>0?$utf8_loss:0).'"></td>';
    
    $o_qty = explode(",",$mat['qty']);
    ...
    switch($mat['arrange']){
        ...
        $color_str .= ($m>0?',':'').($o_qty[$m]>0?$sval['qty']:0);
        $m++;
        }
        break;
        
        case 1:
        $color_str = $color_size[1][$wi_id][$mat['size']]['qty'];
        break;
        
        case 2:
        foreach( $color_size[2][$wi_id][$mat['size']] as $skey => $sval){
        $color_str .= ($m>0?',':'').($o_qty[$m]>0?$sval['qty']:0);
        $m++;
        }
        break;
        ...
        case 3:
        $color_str = $color_size[3][$wi_id]['qty'];
        // echo $color_str.'<br>';
        break;
    ...
    $o_qty = number_format( array_sum(explode(",",$color_str)),0,'','');
    $utf8_qty = $bom_utf8['utf8']['l'][$mat['id']]['qty']?number_format(( $o_qty * $utf8_consump * ( 1 + $utf8_loss / 100 ) ),0,'',''):0;
    $BHTML .= '<td bgcolor="#ffffed" align="right">'.number_format( array_sum(explode(",",$mat['qty'])),0,'',',').' '.$mat['unit'].'</td>';
    $BHTML .= '<td align="right"><input type="text" id="PHP_lots_qty_'.$bom_id.'" name="PHP_lots_qty['.$bom_id.']" style="width:42px" class="styled-input" value="'.($utf8_qty>0?$utf8_qty:0).'"> '.$mat['unit'].'</td>';
    $BHTML .= '<input type="hidden" id="PHP_lots_o_qty_'.$bom_id.'" value="'.$o_qty.'">';
    $BHTML .= '<input type="hidden" name="PHP_lots_o_qty['.$bom_id.']" value="'.(number_format( array_sum(explode(",",$mat['qty'])),0,'','')).'">';
    $BHTML .= '<input type="hidden" name="PHP_lots_po_bom_id['.$bom_id.']" value="'.($utf8_id).'">';
    $BHTML .= '<input type="hidden" name="PHP_lots_unit['.$bom_id.']" value="'.($mat['unit']).'">';
    $BHTML .= '</tr>';
}
...
foreach($bom_acc as $mat){
    ...
    $BHTML .= '<tr id="Bom_TR_List">';
    $BHTML .= '<td><a href="javascript:Show_acc(\''.$mat['acc_code'].'\')" style="cursor:pointer">'.$mat['acc_code'].'</a></td>';
    $BHTML .= '<input type="hidden" name="PHP_acc_code['.$bom_id.']" value="'.($mat['acc_code']).'">';
    $BHTML .= '<input type="hidden" name="PHP_acc_id['.$bom_id.']" value="'.($mat['acc_id']).'">';
    $BHTML .= '<td align="left" bgcolor="#ffffed">'.$mat['acc_name'].'</td>';
    $BHTML .= '<td><input type="text" name="PHP_acc_name['.$bom_id.']" style="width:120px" class="styled-input" value="'.$utf8_name.'"></td>';
    $BHTML .= '<td bgcolor="#ffffed" align="left">'.($utf8_o_color?$utf8_o_color:big52uni($mat['color'])).'</td>';
    $BHTML .= '<input type="hidden" name="PHP_acc_o_color['.$bom_id.']" value="'.big52uni($mat['color']).'">';
    $BHTML .= '<td><input type="text" name="PHP_acc_color['.$bom_id.']" style="width:120px" class="styled-input" value="'.($utf8_color?$utf8_color:big52uni($mat['color'])).'"></td>';
    $BHTML .= '<td align="right" bgcolor="#ffffed">'.number_format($mat['est_1'],3,'',',').' '.$mat['unit'].'</td>';
    $BHTML .= '<td><input type="text" onclick="cell_loss(\'acc\',\''.$bom_id.'\')" onkeyup="cell_loss(\'acc\',\''.$bom_id.'\')" id="PHP_acc_consump_'.$bom_id.'" name="PHP_acc_consump['.$bom_id.']" class="styled-input" style="width:40px" value="'.($utf8_consump>0?$utf8_consump:$mat['est_1']).'"> '.$mat['unit'].'</td>';
    $BHTML .= '<td>'.$mat['size'].'</td>';
    $BHTML .= '<input type="hidden" name="PHP_acc_size['.$bom_id.']" value="'.($mat['size']).'">';
    $BHTML .= '<td><input type="text" onclick="cell_loss(\'acc\',\''.$bom_id.'\')" onkeyup="cell_loss(\'acc\',\''.$bom_id.'\')" id="PHP_acc_loss_'.$bom_id.'" name="PHP_acc_loss['.$bom_id.']" style="width:30px" class="styled-input" value="'.($utf8_loss>0?$utf8_loss:0).'"></td>';
    ...
    $o_qty = number_format( array_sum(explode(",",$color_str)),0,'','');
    $utf8_qty = $bom_utf8['utf8']['a'][$mat['id']]['qty']?number_format(( $o_qty * $utf8_consump * ( 1 + $utf8_loss / 100 ) ),0,'',''):0;
    $BHTML .= '<td bgcolor="#ffffed" align="right">'.number_format( array_sum(explode(",",$mat['qty'])),0,'',',').' '.$mat['unit'].'</td>';
    $BHTML .= '<td align="right"><input type="text" id="PHP_acc_qty_'.$bom_id.'" name="PHP_acc_qty['.$bom_id.']" style="width:42px;" class="styled-input" value="'.($utf8_qty>0?$utf8_qty:0).'"> '.$mat['unit'].'</td>';
    $BHTML .= '<input type="hidden" id="PHP_acc_o_qty_'.$bom_id.'" value="'.$o_qty.'">';
    $BHTML .= '<input type="hidden" name="PHP_acc_o_qty['.$bom_id.']" value="'.(number_format( array_sum(explode(",",$mat['qty'])),0,'','')).'">';
    $BHTML .= '<input type="hidden" name="PHP_acc_po_bom_id['.$bom_id.']" value="'.($utf8_id).'">';
    $BHTML .= '<input type="hidden" name="PHP_acc_unit['.$bom_id.']" value="'.($mat['unit']).'">';
    $BHTML .= '</tr>';
}
$BHTML .= '</table>';
...
#
# ->GET_BOM_ADDED_QTY()
function get_bom_added_qty($wi_id,$size_arr,$color_size,$bom_utf8) {
// print_r($color_size);
// print_r($bom_utf8);
$sql = $this->sql;
...
foreach($bom_lots as $mat){
    ...
    $BHTML .= '<td><a href="javascript:Show_fab(\''.$mat['lots_code'].'\')" style="cursor:pointer">'.$mat['lots_code'].'</a></td>';
    $BHTML .= '<td align="left">'.$utf8_name.'</td>';
    $BHTML .= '<td align="left">'.($utf8_color?$utf8_color:big52uni($mat['color'])).'</td>';
    $BHTML .= '<td align="right">'.($utf8_consump>0?$utf8_consump:$mat['est_1']).' '.$mat['unit'].'</td>';
    $BHTML .= '<td>'.$mat['size'].'</td>';
    $BHTML .= '<td>'.($utf8_loss>0?$utf8_loss:0).'</td>';
    ...
    $o_qty = number_format( array_sum(explode(",",$color_str)),0,'','');
    $utf8_qty = $bom_utf8['utf8']['l'][$mat['id']]['qty']?number_format(( $o_qty * $utf8_consump * ( 1 + $utf8_loss / 100 ) ),0,'',''):0;
    $BHTML .= '<td align="right"><input type="text" id="PHP_lots_qty_'.$bom_id.'" name="PHP_qty['.$bom_id.']" style="width:42px;text-align: right;" class="styled-input" value="'.($utf8_qty>0?$utf8_qty:0).'"> '.$mat['unit'].'</td>';
    $BHTML .= '<input type="hidden" name="PHP_bom_id['.$bom_id.']" value="'.($utf8_id).'">';
    $BHTML .= '</tr>';
}
...
foreach($bom_acc as $mat){
    ...
    $o_qty = number_format( array_sum(explode(",",$color_str)),0,'','');
    $utf8_qty = $utf8_qty ?number_format(( $o_qty * $utf8_consump * ( 1 + $utf8_loss / 100 ) ),0,'',''):0;
    
    // if ( $mat['id'] == '114883') {
        // echo $mat['id'].'|'.$mat['qty'].'|'.$mat['size'].'|'.$mat['arrange'].'|'.$o_qty.'|'.$utf8_consump.'|'.$color_str.'|'.$bom_utf8['utf8']['a'][$mat['id']]['qty'].'<br>';
        // #print_r($color_size[0]);'.$mat['qty'].'|'.$color_str.'
    // }
    
    $BHTML .= '<td align="right"><input type="text" id="PHP_acc_qty_'.$bom_id.'" name="PHP_qty['.$bom_id.']" style="width:42px;text-align: right;" class="styled-input" value="'.($utf8_qty>0?$utf8_qty:0).'"> '.$mat['unit'].'</td>';
    $BHTML .= '<input type="hidden" name="PHP_bom_id['.$bom_id.']" value="'.($utf8_id).'">';
    $BHTML .= '</tr>';
}
$BHTML .= '</table>';
...
#
# ->GET_BOM_EDIT_QTY()
function get_bom_edit_qty($wi_id,$size_arr,$color_size,$bom_utf8) {
// print_r($color_size);
// print_r($bom_utf8);
$sql = $this->sql;
...
foreach($bom_lots as $mat){
    ...
    $o_qty = number_format( array_sum(explode(",",$color_str)),0,'','');
    $utf8_qty = $bom_utf8['utf8']['l'][$mat['id']]['qty']?number_format(( $o_qty * $utf8_consump * ( 1 + $utf8_loss / 100 ) ),0,'',''):0;
    $BHTML .= '<td align="right">'.$o_qty.'<input type="text" id="PHP_lots_qty_'.$bom_id.'" name="PHP_qty['.$bom_id.']" style="width:42px;text-align: right;" class="styled-input" value="'.($utf8_qty>0?$utf8_qty:0).'"> '.$mat['unit'].'</td>';
    $BHTML .= '<input type="hidden" name="PHP_bom_id['.$bom_id.']" value="'.($utf8_id).'">';
    $BHTML .= '</tr>';
}
...
foreach($bom_acc as $mat){
    ...
    $BHTML .= '<td align="right">'.$o_qty.'<input type="text" id="PHP_acc_qty_'.$bom_id.'" name="PHP_qty['.$bom_id.']" style="width:42px;text-align: right;" class="styled-input" value="'.($utf8_qty>0?$utf8_qty:0).'"> '.$mat['unit'].'</td>';
    $BHTML .= '<input type="hidden" name="PHP_bom_id['.$bom_id.']" value="'.($utf8_id).'">';
    $BHTML .= '</tr>';
}
$BHTML .= '</table>';
...
#
# ->GET_BOM_VIEW_QTY()
function get_bom_view_qty($wi_id,$bom_utf8,$bom_utf8_qty) {
...
foreach($bom_lots as $mat){
    ...
    $BHTML .= '<td>'.$mat['size'].'</td>';
    $BHTML .= '<td>'.($utf8_loss>0?$utf8_loss:0).'</td>';
    $BHTML .= '<td align="right">'.$bom_utf8_qty[$utf8_id].' '.($bom_utf8_qty[$utf8_id]?$mat['unit']:'').'</td>';
    $BHTML .= '</tr>';
}
...
#
# :CONFIRM_BOM
function confirm_bom($ord_num) {

$sql = $this->sql;

$q_str = "update `s_order` set 
`pdc_bom_sub_user` = '".$_SESSION['USER']['ADMIN']['id']."' , 
`pdc_bom_sub_date` = NOW() , 
`pdc_status` = '4' 
...
#
# :GET_CARRIER_NUM
function get_carrier_num($carrier) {

$sql = $this->sql;

$fields = false;

$q_str = "SELECT `carrier_num` FROM `po_supl_ship` WHERE `carrier_num` LIKE '%".$carrier."%' AND `status` = '1';";
...
#
# :GET_CARRIER_LIST
function get_carrier_list($dept,$active='') {

$sql = $this->sql;

$fields = false;

$active = ( !empty($active) )? 'AND `incoming_active` = "n" ' : '' ;
...
WHERE 
`dist` = '".$dept."' AND `arrival_user` IS NOT NULL ".$active."
;";
		...
		foreach($get['link_order_qty'][$det_id] as $key => $ord_link_det_qty){
			...
			if($stock_inventory_id){
				...
				foreach($get['link_det_order_qty'][$det_id][$key] as $link_det_ord_num => $link_det_qty){
					...
					$this->append_stock_inventory_link_det($link_det_parm);
				}
...
$q_str = "SELECT `factory` FROM `s_order` WHERE `order_num` LIKE '%".$order_num."%' ;";
...
#
# :GET_ORDER_BOM
function get_order_bom($rn_num, $ver='') {
	...
	for($i=0;$i<$det_size;$i++){
		if($rtn['notice_det'][$i]['mat_cat'] == "l"){
			...
			$row['stock_qty'] = $this->get_stock_qty_by_inventory_id($lots_ver['ver'], $rtn['notice_det'][$i]['mat_cat'], $rtn['notice_det'][$i]['stock_inventory_id']);
			$row['unclaimed_qty'] = $rtn['notice_det'][$i]['qty'] - $row['send_qty'] <= $row['stock_qty'] ? $rtn['notice_det'][$i]['qty'] - $row['send_qty'] : $row['stock_qty'];
...
$q_str = "SELECT `fty` FROM `requi_notify` WHERE `rn_num` LIKE '%".$notice_num."%' ;";
...
$query .= !empty($parm['notice_num']) ? " AND `rn_num` LIKE '%".$parm['notice_num']."%' " : "" ;
...
function get_po_ship_det_group_by_mat($id) {
	...
	while ($row = $sql->fetch($q_result)) {
		...
		$q_str = "SELECT ap_det.id, ap_det.order_num, ap_det.bom_id, ap_det.po_qty, ap_det.po_unit,
						 sum(stock_inventory_link_det.qty) as i_qtyed
					FROM ap_det left join stock_inventory_link_det on stock_inventory_link_det.bom_id = ap_det.bom_id and stock_inventory_link_det.bl_num = '".$row['bl_num']."'
...
function update_cat_qty_after_send($fty,$ver, $bom_id, $cat, $stock_inventory_id, $qty) {
	...
	$q_str = "select sum(qty) as qty
			  from stock_inventory_link_det
			  where stock_inventory_id = ".$stock_inventory_id;

	$q_result = $sql->query($q_str);
	$row = $sql->fetch($q_result);
	if($row['qty'] <= $qty){
		$q_str = "update stock_inventory_link_det
				  set send_qty = qty
				  where stock_inventory_id = ".$stock_inventory_id;
