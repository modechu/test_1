...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function search($parm=0, $dept='',$limit_entries=0) {

$sql = $this->sql;
$argv = $_SESSION['parm'];   //將所有的 globals 都抓入$argv
$srh = new SEARCH();
...
{			
	if($user_dept == $sale_f_mang[$i]) 	$srh->add_where_condition("receive.dept LIKE '".$sale_mang[$i]."%'", "PHP_dept",$sale_mang[$i],"");		
}

//部門 : 業務部門
$sales_dept = $GLOBALS['SALES_DEPT'];
if ($team == 'MD')	$srh->add_where_condition("receive.dept = '$user_dept'", "PHP_dept","$user_dept","Search Dept = [ $user_dept ]");
for ($i=0; $i< sizeof($sales_dept); $i++)
{			
	if($user_dept == $sales_dept[$i] && $team <> 'MD') 	$srh->add_where_condition("receive.dept = '$user_dept'", "PHP_dept","$user_dept","Search Dept =[ $user_dept ]");		
}	
...
// 'PHP_ap_user'
	...
	$mesg = '';
	if ($str = strtoupper($argv['PHP_rcv_date']) ) {
		$srh->add_where_condition("receive.rcv_date LIKE '%$str%'", "PHP_rcv_date",$str);
		$mesg.= "  PA# : [ $str ]. ";
	}

	if ($str = $argv['PHP_sup_code'] ) {
		$srh->add_where_condition("receive.sup_code = '$str'", "PHP_sup_code",$str);
		$mesg.= "  Supl. = [ $str ]. ";
	}

	if ($str = $argv['PHP_ap_user'] ) {
		$srh->add_where_condition("receive.ap_user = '%$str%'", "PHP_ap_user",$str);
		$mesg.= "  Ship = [ $str ]. ";
	}

	// if ($str = $argv['PHP_SCH_cust'] ) {
		// $srh->add_where_condition("receive.cust = '$str'", "PHP_SCH_cust",$str);
		// $mesg.= "  Cust. = [ $str ]. ";
	// }
...
*/
			$q_str = "INSERT INTO ap_det (ap_num,bom_id,mat_cat,eta,unit,ap_qty) 
				  VALUES('".
							$parm['ap_num']."','".
							$parm['mat_id']."','".
							$parm['mat_cat']."','".
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function get_lots_det($id=0,$support=0) {
		...
		while ($row1 = $sql->fetch($q_result)) {
			...
			if($row1['pp_mark'] == 0)
			{
				// $row1['pp'][] = $this->get_po_qty($row1['id'],'l',$row1['ap_mark']);	
			} else {
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#	->get_special_ap($id=0)	抓出指定記錄 bom 主料相關資料 RETURN $row[]
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function get_special_ap($id=0,$mat) {
		$ap_num=array();
		$sql = $this->sql;
		$q_str="SELECT DISTINCT ap.ap_num, ap.status, ap_det.eta, ap_det.ap_qty, ap_det.unit, ap_det.po_qty, ap_det.po_unit, ap.po_num 
						FROM ap, ap_det 
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function get_po_qty($id=0,$mat, $num) {
		$ap_num=array();
		$sql = $this->sql;
		$q_str="SELECT ap_det.po_qty as qty, po_unit as unit, unit as pa_unit, po_eta as eta, ap_det.ap_qty , ap.status, ap.po_num ,ap.ap_num
						FROM ap_det, ap
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function get_pp_qty($id=0,$mat, $code,$color) {
		$ap_num=array();
		$sql = $this->sql;
		$q_str="SELECT ap_special.po_qty as qty, po_unit as unit, unit as pa_unit, eta, ap_special.ap_qty , ap.status, ap.po_num ,ap.ap_num
						FROM ap_special , ap
