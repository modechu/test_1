...
// Created: 	2003-09-17
// Ver:		$Id: jpgraph_windrose.php 1091 2009-01-18 22:57:40Z ljp $
//
...
class WindrosePlotScale {
    ...
    function WindrosePlotScale($aData) {
	$max=0;
	$totlegsum = 0;
	$maxnum=0;
	$this->iZeroSum=0;
...
class WindrosePlot {
    ...
    private $iCircleGridStyle='solid';
    ...
    private $iLabelFormatString='%.1f°', $iLabels=array();
    private $iLabelPositioning = LBLPOSITION_EDGE;
    private $iColor='white';
    private $iShowBox=false, $iBoxColor='black',$iBoxWeight=1;
    private $iOrdinalEncoding=KEYENCODING_ANTICLOCKWISE;
    public $legend=null;

    function WindrosePlot($aData) {
	$this->iData = $aData;
    ...
    function SetBox($aColor='black',$aWeight=1,$aShow=true) {	
	$this->iShowBox = $aShow ;
	...
	//
	//$aImg->SetColor($this->iGridColor1);
	...
	if( $aIsRegRose ) {
		...
		// Weight the leg we will use more to give poreference
		...
		//echo "i=$i, a=$try_a : d1=$d1, d2=$d2, w=$w [$kj,$kl]<br>&nbsp;&nbsp --> max=$max, found=$found<br>\n";
...
//('You have specified the same direction twice, once with an angle and once with a compass direction ('.$a*180/M_PI.' degrees.)');
	...
	for($i=0; $i < $num; ++$i ) {
	    ...
	    // Determine the label
	    $da = floor($a*180/M_PI);
	    if( !empty($this->iLabels[$keys[$i]/*$da*/]) ) {
		$lbl = $this->iLabels[$keys[$i]/*$da*/];
	    }
	    else {
		$lbl = sprintf($this->iLabelFormatString,$a*180/M_PI);
	    }
	...
	// Plot radial grid lines and remember the end position
	// and the angle for later use when plotting the labels
	if( $this->iType == WINDROSE_TYPE4 ) $num = 4;
	elseif( $this->iType == WINDROSE_TYPE8 ) $num = 8;
	elseif( $this->iType == WINDROSE_TYPE16 ) $num = 16;
	else {
...
//---------------
// CONSTRUCTOR
    function WindroseGraph($width=300,$height=200,$cachedName="",$timeout=0,$inline=1) {
	...
	if( $this->iIcons != null ) {
	    $n = count($this->iIcons);
	    for( $i=0; $i < $n; ++$i ) {
	        // Since Windorse doesn't have any linear scale the position of
...
// PUBLIC METHODS	
	...
	elseif( $cl instanceof IconPlot ) 
	    $this->AddIcon($aObj);
	elseif( !($cl instanceof WindrosePlot) )
	...
	for($i=0; $i < $n ; ++$i) {
	    $wp = $this->plots[$i];
	    if( $wp->iX > 0 && $wp->iX < 1 ) 
		$wp->iX = round( $this->img->width * $wp->iX ) ;
	    if( $wp->iY > 0 && $wp->iY < 1 ) 
		$wp->iY = round( $this->img->height * $wp->iY ) ;
	    if( $wp->iSize > 0 && $wp->iSize < 1 )
		$wp->iSize *= min($this->img->width,$this->img->height);
	    if( $wp->iCenterSize > 0 && $wp->iCenterSize < 1 )
		$wp->iCenterSize *= $wp->iSize;
	    
	    $wp->scale->AutoScale(($wp->iSize-$wp->iCenterSize)/2,round(2.5*$wp->scale->iFontSize));
