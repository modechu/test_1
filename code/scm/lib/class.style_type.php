...
#	->del($id)					刪除 資料錄
#	->get_fields($n_field,$where_str="")	取出全部 STYLE TYPE 的 $n_field 置入arry return
#
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function add($parm) {
		...
		if ($sql->num_rows($q_result) ) {
			$this->msg->add("SORRY ! 這個款式類別已經登錄了，請換別的類別或更改原內容!");
			return false;    	    
		}
					# 加入資料庫
		...
		$style_type_id = $sql->insert_id();  //取出 新的 id
		$this->msg->add("成功 新增款式類別 : [".$parm['style_type']."]。") ;
		return $style_type_id;

	} // end func
	
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#	->search($mode=0)	搜尋 款式類別	 
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function search($mode=0) {
		...
		$op['style_type'] = $result;  // 資料錄 拋入 $op
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#	->get($id=0, style_type=0)	抓出指定記錄內資料 RETURN $row[]
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function get($id=0, $style_type=0) {
		...
		if ($id)	{
			...
			$this->msg->add("Error ! 請指明 款式類別 在資料庫內的 ID.");		    
			return false;
		}
...
###########  撿查 新加入的 style_type 是否為唯一 [note:可能會找到原來的自己 #####
$q_str2 = "SELECT id FROM style_type WHERE style_type='".$parm['style_type']."'"." AND id <> '".$parm['id']."'";
		...
		if ($sql->num_rows($q_result2) ) {
			$this->msg->add("SORRY ! 這個款式類別已經登錄資料庫內，請換別的名稱或是更新原記錄!");
			return false;    	    
		}
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#	->update_field($parm)		更新 款式類別資料內 某個單一欄位
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function del($id) {
		...
		$sql = $this->sql;
		if (!$id) {
			$this->msg->add("Error !請指明 style_type 資料表的 ID.");		    
			return false;
		}
