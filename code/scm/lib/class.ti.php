...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function search($mode=0, $where_str="",$sort_str="item , id") {
		...
		$q_header = "SELECT * FROM ti ".$where_str;
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function get($id=0, $acc_code=0) {
		...
		if ($id)	{
			...
			$this->msg->add("Error ! Please point illustrate ID.");		    
			return false;
		}
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function update_field($parm) {
		...
		$q_str = "UPDATE ti SET ".$parm['field_name']."='".$parm['field_value']."',date='".date('Y-m-d')."',ti_time='".date('H:i:s')."'  WHERE id='".$parm['id']."'";
//echo $q_str."<BR>";
		if (!$q_result = $sql->query($q_str)) {
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function del($id,$mode=0) {
		...
		if($mode){
			$q_str = "DELETE FROM ti WHERE wi_id='$id' ";
		}else{
			$q_str = "DELETE FROM ti WHERE id='$id' ";
		}
		if (!$q_result = $sql->query($q_str)) {
			$this->msg->add("Error ! Can't access database !");
			$this->msg->merge($sql->msg);
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function get_fields($n_field,$where_str="") {
		$sql = $this->sql;
		$fields = array();
		$q_str = "SELECT ".$n_field." FROM ti ".$where_str;
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function del_group($id,$sub_item) {
		...
		$q_str = "DELETE FROM ti WHERE wi_id='$id' AND sub_item='$sub_item' ";
		if (!$q_result = $sql->query($q_str)) {
			$this->msg->add("Error ! Can't access database !");
			$this->msg->merge($sql->msg);
			return false;    
