...
class PDF_fty_bom_L extends PDF_Chinese 	{
...
function Header() {
    ...
    $this->RotatedText(100,150,$ary_title['PHP_id'],30);
	//Logo
...
function Table_2_title($title,$header1,$w1,$use_title,$R,$G,$B)
{
  ...
  $this->Ln();
	$this->SetX(10);	
  for($i=0;$i<(count($header1)-2);$i++)
  {
     if($header1[$i] == 'size' || $header1[$i] == 'Consump.')
     {
  ...
  foreach($use_title as $key => $value)
  {
    ...
    {
			if($j > 6 && strlen($use_title[$key]['name'][$i]) > 8)
				$use_title[$key]['name'][$i] = substr($use_title[$key]['name'][$i],0,8)."...";
			if($j < 6 && strlen($use_title[$key]['name'][$i]) > 22)
				$use_title[$key]['name'][$i] = substr($use_title[$key]['name'][$i],0,22)."...";
  ...
  $this->Cell($w1[(count($header1)-1)],10,$header1[(count($header1)-1)],1,0,'C',1);
  ...
  foreach($use_title as $key => $value)
  {
    ...
    {
        $this->Cell($color_size,5,$use_title[$key]['qty'][$i].' pcs',1,0,'C',0);
 		}
 	}
  $this->Ln();
...
function Table_2($row,$w1) {
	...
	if(isset($row['lots_name']))$this->Cell($w1[0],5,$row['lots_name'],1,0,'L',0);
	//$this->Cell($w1[1],5,$row['use_for'],1,0,'L',0);
	$this->Cell($w1[1],5,$row['est_1'].$row['unit'],1,0,'R',0);	
	//$this->Cell($w1[3],5,$row['color'],1,0,'R',0);	
	$this->Cell($w1[2],5,$row['size'],1,0,'L',0);
	$this->Cell($w1[3],5,'',1,0,'L',0);
	$this->SetFont('Arial','B',8);

	$size_qty = 174/sizeof($row['color_use']);
	for($i=0; $i<sizeof($row['color_use']); $i++)
	{
		if(sizeof($row['color_use']) > 6 && strlen($row['color_use'][$i]) > 8)
			$row['color_use'][$i]=substr($row['color_use'][$i],0,8).'...';
		else if(sizeof($row['color_use']) > 1 && strlen($row['color_use'][$i]) > 22)
			$row['color_use'][$i]=substr($row['color_use'][$i],0,22).'...';
			
		$this->Cell($size_qty,5,$row['color_use'][$i],1,0,'L',0);

	}
	$this->SetFont('BIG5','',8);
	$this->Cell($w1[4],5,$row['use_for'],1,0,'L',0);
	$this->Ln();
