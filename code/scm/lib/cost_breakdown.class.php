...
function get_order_amt_etd($fty,$year,$month){
# 不包含工廠
$sql = $this->sql;
...
if( $result = $sql->query($query) ){
    ...
    while($row = $sql->fetch($result)){
        ...
        $other_cost = $special + $others + $cm;
        
        $sum += $unit_cost * $qty + $handling_fee * $qty;
        $other_sum += $other_cost * $qty + $handling_fee * $qty;

        # PO Cost Array
        $order_arr[$row['order_num']]['qty'] = $row['ord_qty'];
        $order_arr[$row['order_num']]['p_qty'] += $row['qty'];
...
$rows['sum'] = $sum;
$rows['other_sum'] = $other_sum;

$rows['order_arr'] = $order_arr;
...
function get_po_cost_etd($year,$month,$order_arr){
# 不包含工廠
$sql = $this->sql;
...
$rate = $GLOBALS['rate']->get_rate_date($year.'-'.$month.'-15');
// echo '<br>'.$rate.'<br>';
// echo '<br>'.$year.'-'.$month.'<br>';
foreach($order_arr as $order_num => $val) {
    ...
    // `ap_det`.`amount` ,
    // `ap_det`.`order_num` 
    // FROM `ap` , `ap_det` , `apb` , `apb_det`
    // WHERE 
    // `ap`.`ap_num` = `ap_det`.`ap_num` AND 
    // `ap_det`.`ap_num` = `apb_det`.`ap_num` AND 
    // `ap_det`.`po_spare` = `apb_det`.`po_id` AND 
    // `ap_det`.`prics` = `apb_det`.`uprice` AND 
    // `apb`.`rcv_num` = `apb_det`.`rcv_num` AND 
    // `ap`.`currency` != `apb`.`currency` AND 
    // `ap_det`.`order_num` = '".$order_num."' AND 
    // `ap`.`status` = '12' ";
    // $query .= " ;";
    // $aps = array();
    // $result = $sql->query($query);
    // while($row = $sql->fetch($result)){
        // $aps[$row['amount']][$row['amount']] = 
        // echo $row['ap_num'].' ~ '.$row['rcv_num'].'<br>';
    // }
    // echo $query.'<br>';
    ...
    if( $result = $sql->query($query) ){
        ...
        while($row = $sql->fetch($result)){
            ...
            $currency_arr[$row['currency']]['amount'] += $row['amount'];
            $ap_num[$row['ap_num']] = $row['ap_num'];

        }

        foreach($currency_arr as $currency => $value){
        // echo $order_num.' : '.$value['ap_num'].'<br>';
        
            if( $currency == 'USD' ) {
                $po_sum += ( $value['amount'] / $val['qty'] ) * $val['p_qty'] ;
                // echo $currency.' : '.$value['amount'].' / '.$val['qty'].' * '.$val['p_qty'] .'<br>';
            } else if ( $currency == 'NTD' ) {
                $po_sum += ( ( $value['amount'] / $rate['USD'] ) / $val['qty'] ) * $val['p_qty'] ;
                // echo $currency.' : '.$value['amount'].' / '.$rate['USD'].' / '.$val['qty'].' * '.$val['p_qty'] .'<br>';
            } else {
                $po_sum += ( ( ( $value['amount'] * $rate[$currency] ) / $rate['USD'] ) / $val['qty'] ) * $val['p_qty'] ;
                // echo $currency.' : '.$value['amount'].' * '.$rate[$currency].' / '.$rate['USD'].' / '.$val['qty'].' * '.$val['p_qty'] .'<br>';
            }
        // echo ' USD : '.$po_sum.'<p>';
        }
...
function get_exp_order_amt_etd($fty,$year,$month,$order_arr){
# 不包含工廠
$sql = $this->sql;
...
FROM `s_order` , `shipping_doc` , `shipping_doc_qty`
WHERE 
`shipping_doc`.`id` = `shipping_doc_qty`.`s_id` AND 
`shipping_doc`.`factory` = '".$fty."' AND 
`s_order`.`order_num` = `shipping_doc_qty`.`ord_num` AND 
SUBSTRING(`shipping_doc_qty`.`ord_num`,1,1) != 'L' AND 
`shipping_doc`.`ship_date` LIKE '".$year.'-'.$month.'-%'."'
...
if( $result = $sql->query($query) ){
    ...
    while($row = $sql->fetch($result)){
        ...
        WHERE 
        `shipping_doc_qty`.`ord_num` = '".$row['ord_num']."' 
        GROUP BY `shipping_doc_qty`.`ord_num`
        ";
        $query .= " ;";
        $results = $sql->query($query);
        if( $rowr = $sql->fetch($results) ){
            $ttl_amount = $rowr['amount'];
        } else {
            $ttl_amount = 0;
        }
        
        $exp_sum += ( $ttl_amount / $row['qty'] ) * $row['ship_qty'];
        
        $order_arr[$row['ord_num']]['qty'] = $row['qty'];
        $order_arr[$row['ord_num']]['ship_qty'] += $row['ship_qty'];
    }
}
...
function get_exp_ship_amt_etd($fty,$year,$month){
# 不包含工廠
$sql = $this->sql;

$po_sum = $po_pc = $po_su = 0;
$rows = $order_arr = array();

$rate = $GLOBALS['rate']->get_rate_date($year.'-'.$month.'-15');

#調整結帳時間，由201504開始，之前抓取的資料方式保持抓整月份，201504從01到25截止，201505從20150426~20150525
if( $year == '2015' && $month == '04' ) {
    $where = " `shipping_doc`.`ship_date` between '".$year.'-'.$month."-01' AND '".$year.'-'.$month."-25' AND ";
} else if($year=='2015'&&$month>'04'){
    $before = date("Y-m", mktime(0, 0, 0, $month-1, 01, $year));
    $where = " `shipping_doc`.`ship_date` between '".$before."-26' AND '".$year.'-'.$month."-25' AND ";
} else {
    $where = " `shipping_doc`.`ship_date` like '".($year.'-'.$month)."%' AND ";
}
    
$query  = " SELECT 
`shipping_doc`.`ttl_amt` , 
sum(`shipping_doc_qty`.`ttl_qty`) as `ttl_qty` , 
sum(`shipping_doc_qty`.`amount`) as `amount` , 
`shipping_doc_qty`.`ord_num` , 
...
FROM `shipping_doc` , `shipping_doc_qty` LEFT JOIN `s_order` ON ( `s_order`.`order_num` = `shipping_doc_qty`.`ord_num` AND SUBSTRING(`shipping_doc_qty`.`ord_num`,1,1) != '".substr($fty,0,1)."' ) 

WHERE 
`shipping_doc`.`id` = `shipping_doc_qty`.`s_id` AND 
".$where."
`shipping_doc`.`factory` = '".$fty."' 
GROUP BY `shipping_doc_qty`.`ord_num`
";
...
if( $result = $sql->query($query) ){
    ...
    while($row = $sql->fetch($result)){
        ...
        $po_sum += $row['amount'];
        $order_arr[$row['ord_num']]['qty'] += $row['qty'];
        $order_arr[$row['ord_num']]['ship_qty'] += $row['ttl_qty'];
        $order_arr[$row['ord_num']]['ship_su'] += $row['ttl_qty']*$ie;
        $order_arr[$row['ord_num']]['cost'] = (($row['org_cm']*$ie)+$row['emb']+$row['wash']+$row['oth']+$row['handling_fee']);

        $po_pc += $row['ttl_qty'];
        $po_su += ($row['ttl_qty']*$ie);
    }
...
function get_exp_cost_etd($year,$month,$order_arr){
# 不包含工廠
$sql = $this->sql;
...
// if($month=='03')
// print_r($order_arr);
$did_array = $rcv_num_arr = array();
$ttl_qty = $ttl_ship_qty = $ttl_ship_su = $ttl_amount = $ttl_cost = $ttl_cost_lots_amount = $ttl_cost_acc_amount = 0;
...
foreach($order_arr as $order_num => $val) {
    ...
    FROM `apb` , `apb_det` , `apb_po_link` LEFT JOIN `apb_oth_cost` ON ( `apb`.`rcv_num` = `apb_oth_cost`.`apb_num` AND `apb_oth_cost`.`item` != 'T/T before shipment' )
    WHERE 
    `apb`.`rcv_num` = `apb_det`.`rcv_num` AND 
    `apb_det`.`id` = `apb_po_link`.`rcv_id` AND 
    `apb_po_link`.`ord_num` = '".$order_num."' AND 
    ( ( `apb`.`payment` != 'T/T before shipment' AND `apb`.`status` = '2' ) OR ( `apb`.`payment` = 'T/T before shipment' AND `apb`.`status` = '4' ) )
    GROUP BY `apb_po_link`.`id`
    ";
    ...
    if( $result = $sql->query($query) ){
        ...
        $cost = $oth_cost = $fab_acc_apb_sum = $po_lots_sum = $po_acc_sum = $ttl_sum = $amount = 0;
        while($row = $sql->fetch($result)){
            ...
            $amount = $row['uprice'] * $qty;
            if( $currency == 'USD' ) {
                #轉換美金單價
                $uprice = $uprice;
                // if( $row['cost'] > 0 && !in_array($rcv_num,$did_array) ) $oth_cost = ( $row['cost'] / $row['qtys'] ) * $row['qty'];
                if( $row['cost'] > 0 && !in_array($rcv_num,$did_array) ) $ttl_cost += $row['cost'];
                #分攤物料出口費用
                // $price = ( $uprice * $qty / $val['qty'] ) * $val['ship_qty'] ;
                // echo $currency.' : '.$uprice * $qty.' / '.$val['qty'].' * '.$val['ship_qty'] .' = '.( $uprice * $qty / $val['qty'] ) * $val['ship_qty'].'<br>';
            } else if ( $currency == 'NTD' ) {
                #轉換美金單價
                $uprice = $uprice / $rate['USD'];
                // if( $row['cost'] > 0 && !in_array($rcv_num,$did_array) ) $oth_cost = ( $row['cost'] / $rate['USD'] / $row['qtys'] ) * $row['qty'];
                if( $row['cost'] > 0 && !in_array($rcv_num,$did_array) ) $ttl_cost += $row['cost'] / $rate['USD'];
                #分攤物料出口費用
                // $price = ( ( $uprice * $qty / $rate['USD'] ) / $val['qty'] ) * $val['ship_qty'] ;
                // echo $currency.' : '.$uprice * $qty.' / '.$rate['USD'].' / '.$val['qty'].' * '.$val['ship_qty'] .' = '.( ( $uprice * $qty / $rate['USD'] ) / $val['qty'] ) * $val['ship_qty'].'<br>';
            } else {
                #轉換美金單價
                $uprice = ( $uprice * $rate[$currency] ) / $rate['USD'];
                // if( $row['cost'] > 0 && !in_array($rcv_num,$did_array)) $oth_cost = ( ( $row['cost'] * $rate[$currency] ) / $rate['USD'] / $row['qtys'] ) * $row['qty'];
                if( $row['cost'] > 0 && !in_array($rcv_num,$did_array)) $ttl_cost += ( $row['cost'] * $rate[$currency] ) / $rate['USD'];
                #分攤物料出口費用
                // $price = ( ( ( $uprice * $qty * $rate[$currency] ) / $rate['USD'] ) / $val['qty'] ) * $val['ship_qty'] ;
                // if($order_num == 'ACH14-0910')
                // echo $currency.' : '.$uprice * $qty.' * '.$rate[$currency].' / '.$rate['USD'].' / '.$val['qty'].' * '.$val['ship_qty'] .' = '.( ( ( $uprice * $qty * $rate[$currency] ) / $rate['USD'] ) / $val['qty'] ) * $val['ship_qty'].'<br>';
            }
            $did_array[] = $rcv_num;
            
            #加總物料費用
            $fab_acc_apb_sum += $uprice * $qty;
            #分攤主、副料成本
            if( substr($mat_code,0,1) == 'F' ){
                $po_lots_sum += $uprice * $qty ;
            } else {
                $po_acc_sum += $uprice * $qty ;
                // if($order_num == 'ACH14-0910')
                // echo $mat_code.' : '.$uprice * $qty.' : '.$po_acc_sum.'<br>';
            }
        }
        
        // $oth_cost = 0;
        // array_unique($rcv_num_arr);
        // foreach($rcv_num_arr as $rcv_num){
            
            // $querys  = " SELECT sum(`apb_oth_cost`.`cost`) as `oth_cost` FROM `apb_oth_cost` WHERE `apb_oth_cost`.`apb_num` = '".$rcv_num."' GROUP BY `apb_oth_cost`.`apb_num` ";
            // if( $results = $sql->query($querys) ){
                // $rowss = $sql->fetch($results);
                // $oth_cost += $rowss['oth_cost'];
            // }
        // }
        
        // $row_arr['oth_cost'] = $oth_cost;
        $row_arr['order_num'] = $order_num;
        // $row_arr['amount'] = ( $fab_acc_apb_sum / $val['qty'] * $val['ship_qty'] ) + ( $val['cost'] * $val['ship_qty'] ) + $oth_cost;
        $row_arr['amount'] = ( $fab_acc_apb_sum / $val['qty'] * $val['ship_qty'] ) + ( $val['cost'] * $val['ship_qty'] );
        // $row_arr['amount'] = $po_sum + $val['cost'] * $val['ship_qty'] + $row_arr['oth_cost'];
        
        $row_arr['qty'] = $val['qty'];
        $row_arr['ship_qty'] = $val['ship_qty'];
        $row_arr['ship_su'] = $val['ship_su'];
        $row_arr['cost'] = $val['cost'];
        $row_arr['emb'] = $val['emb'];
        $row_arr['wash'] = $val['wash'];
        $row_arr['oth'] = $val['oth'];
        
        // $po_sum += $val['cost']*$val['ship_qty'];
        $rows['arr'][] = $row_arr;
        
        $ttl_ship_qty += $val['ship_qty'];
        $ttl_ship_su += $val['ship_su'];
        $ttl_amount += $row_arr['amount'];
        
    }
}

$rows['ttl_ship_qty'] = $ttl_ship_qty;
// $rows['ttl_ship_su'] = $ttl_ship_su;
// $rows['ttl_amount'] = $ttl_amount + $ttl_cost;
// $rows['sum'] = $ttl_amount + $ttl_cost;
$rows['ttl_amount'] = $ttl_amount;
$rows['sum'] = $ttl_amount;
...
function get_output_su_etd($fty,$year,$month){
...
SUBSTRING(`saw_out_put`.`ord_num`,1,1) != 'L' AND 
`saw_out_put`.`out_date` LIKE '".$year.'-'.$month."%' 
";
$query .= " ;";
// echo $query.'<br>';
if( $result = $sql->query($query) ){
...
function get_exp_ship($fty, $year, $month){
# 不包含工廠
$sql = $this->sql;
...
function get_exp_cost($fty, $year, $month){
# 不包含工廠
$sql = $this->sql;
...
if( $result = $sql->query($query) ){
    ...
    while($row = $sql->fetch($result)){
        ...
        $order_arr[$row['ord_num']]['ship_qty'] += $row['ttl_qty'];
        $order_arr[$row['ord_num']]['ship_su'] += $row['ttl_qty']*$ie;
        $order_arr[$row['ord_num']]['ttl_cost'] = (($row['org_cm']*$ie)+$row['emb']+$row['wash']+$row['oth']+$row['handling_fee']);
        
        $order_arr[$row['ord_num']]['ie'] = $ie;
        $order_arr[$row['ord_num']]['org_cm'] = $row['org_cm'];
        $order_arr[$row['ord_num']]['emb'] = $row['emb'];
        $order_arr[$row['ord_num']]['wash'] = $row['wash'];
        $order_arr[$row['ord_num']]['oth'] = $row['oth'];
        
        $order_arr[$row['ord_num']]['handling_fee'] = $row['handling_fee'];
        // $order_arr[$row['ord_num']]['style'] = $row['style'];
...
foreach($order_arr as $order_num => $val) {
    ...
    `apb_det`.`id` = `apb_po_link`.`rcv_id` AND 
    `apb_po_link`.`ord_num` = '".$order_num."' AND 
    ( ( `apb`.`payment` != 'T/T before shipment' AND `apb`.`status` = '2' ) OR ( `apb`.`payment` = 'T/T before shipment' AND `apb`.`status` = '4' ) ) GROUP BY `apb_po_link`.`id`
    ";

    $query .= " ;";

    // echo $query.'<br>';
        // if( $order_num == 'ACB14-0977' ) {
            // echo $query.'<br>';
            // echo $order_num.':'.$po_sum.'<br>';
        // }
    if( $result = $sql->query($query) ){
        ...
        while($row = $sql->fetch($result)){
            ...
            if( $row['cost'] > 0 && !in_array($rcv_num,$did_array)){
                $did_array[] = $rcv_num;
                // $rcv_num_arr[$rcv_num] = $rcv_num;
                
                // $rcv_num_arr[$rcv_num]['inv_price'] = $row['inv_price'];
            }
            // $did_array[] = $row['rcv_num'];
            // $rcv_num_arr[$rcv_num]['inv_price'] = $row['inv_price'];
            ...
            if( substr($mat_code,0,1) == 'F' ){
                $po_lots_sum += $uprice * $qty ;
                // $rcv_num_arr[$rcv_num]
                // if ( $row['cost'] > 0 && !in_array($rcv_num,$did_array['rcv_num']) && $order_num == 'ACB14-0977' ) {
                    // $rcv_num_arr['fab']['qty'] += $row['qty'];
                    
                    // $rcv_num_arr['apb']['inv_price'] += $row['inv_price'];
                    // $did_array['rcv_num'][$rcv_num] = $rcv_num;
                // }
                // if ( $row['cost'] > 0 && $order_num == 'ACB14-0977' ) {
                    // $did_array[$rcv_num]['qty'][] = $row['qty'];
                    // $did_array[$order_num]['ttl_qty'] += $row['qty'];
                // }
            } else {
                $po_acc_sum += $uprice * $qty ;
                // if( $row['cost'] > 0 && !in_array($rcv_num,$did_array['rcv_num']) && $order_num == 'ACB14-0977'){
                    // $rcv_num_arr['acc']['inv_price'] += $row['inv_price'];
                    // $rcv_num_arr['apb']['inv_price'] += $row['inv_price'];
                    // $did_array['rcv_num'][$rcv_num] = $rcv_num;
                // }
                // if($order_num == 'ACH14-0910')
            ...
            // echo $mat_code.' : '.$uprice.' * '.$qty.' = '.$uprice*$qty.'     === '.$po_lots_sum.'    === '.$po_acc_sum.'<br>';
            // $rcv_num_arr[$rcv_num] = $rcv_num;
        }
        ...
        // foreach($rcv_num_arr as $rcv_num){
            ...
            // if( $results = $sql->query($querys) ){
                // $rowss = $sql->fetch($results);
                // $oth_cost += $rowss['oth_cost'];
                // if($order_num == 'ACH14-0910' && $rowss['oth_cost'] > 0){
                    // print_r($rcv_num_arr);
                    // echo $querys.'<br>';
                    // echo $oth_cost.'<br>';
                // }
        ...
        $row_arr['oth_cost'] = $oth_cost;
        // if($order_num == 'ACH14-0910')
        // echo $row_arr['oth_cost'].'<br>';
        $row_arr['order_num'] = $order_num;
        
        // $row_arr['uprice'] = $fab_acc_apb_sum;
        
        // $row_arr['amount'] = ( $fab_acc_apb_sum / $val['qty'] * $val['ship_qty'] ) + ( $val['ttl_cost'] * $val['ship_qty'] ) + $oth_cost;
        $row_arr['amount'] = ( $fab_acc_apb_sum / $val['qty'] * $val['ship_qty'] ) + ( $val['ttl_cost'] * $val['ship_qty'] );
        // if($order_num == 'ACH14-0910'){
        // echo $row_arr['amount'].'='.$fab_acc_apb_sum.'+('.$val['ttl_cost'].'*'.$val['ship_qty'].')+'.$oth_cost.'<br>';
        // echo $row_arr['amount'].'='.$fab_acc_apb_sum.'+('.$po_lots_sum.')'.'+('.$po_acc_sum.')+'.$oth_cost.'<br>';
        // echo $row_arr['amount'].'<br>';
        // }
        $row_arr['qty'] = $val['qty'];
        $row_arr['ship_qty'] = $val['ship_qty'];
        $row_arr['ship_su'] = $val['ship_su'];
        $row_arr['cost'] = $oth_cost;
        $row_arr['emb'] = $val['emb'];
        $row_arr['wash'] = $val['wash'];
        $row_arr['oth'] = $val['oth'];
        
        $row_arr['org_cm'] = $val['org_cm'] * $val['ie'];
        $row_arr['cm'] = $val['org_cm'];
        $row_arr['fee'] = $val['handling_fee'];

        // 主副料
        $row_arr['cost_lots'] = $po_lots_sum / $val['qty'];
        $row_arr['cost_acc'] = $po_acc_sum / $val['qty'];
        // 單件成本
        $row_arr['cost_unit'] = $row_arr['cost_lots'] + $row_arr['cost_acc'] + ( $row_arr['org_cm'] ) + $row_arr['fee'] + $row_arr['emb'] + $row_arr['wash'] + $row_arr['oth'];

        // $fab_acc_apb_sum += $val['cost']*$val['ship_qty'];
        $rows['arr'][] = $row_arr;
        ...
        $ttl_ship_su += $val['ship_su'];
        $ttl_amount += $row_arr['amount'];
        $ttl_cost_lots_amount += $row_arr['cost_lots'];
        $ttl_cost_acc_amount += $row_arr['cost_acc'];
        $ttl_oth_cost_amount += $row_arr['oth_cost'];
    }
    // if( $order_num == 'ACH14-0910') {
        // echo 'ttl = '.$ttl_sum.'<br>';
        // echo 'cost_lots : '.$po_lots_sum.'<br>';
        // echo 'cost_acc : '.$po_acc_sum.'<br>';
    // }    
}
// sort($did_array);
// print_r($did_array);
...
$rows['ttl_ship_su'] = $ttl_ship_su;
$rows['ttl_amount'] = $ttl_amount;
$rows['ttl_cost_lots_amount'] = $ttl_cost_lots_amount;
$rows['ttl_cost_acc_amount'] = $ttl_cost_acc_amount;
$rows['ttl'] = $ttl_amount+$ttl_cost;
$rows['ttl_oth_cost_amount'] = $ttl_cost;
// $rows['sum'] = $fab_acc_apb_sum;
