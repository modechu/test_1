...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function add_main() {					
		$sql = $this->sql;
		$today = date('Ymd');
		$q_str = "SELECT num FROM po_ship where num like '".$today."%' order by num desc limit 1";
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function add_det($parm1) {					
		$sql = $this->sql;
					...
					# 加入資料庫(2007.03.02加入尺吋資料)
		$q_str = "INSERT INTO po_ship_det (ship_id,inv_num,qty,ap_num,bom_id,mat_id,color,size) 
				  VALUES('".
							$parm1['ship_id']."','".
							$parm1['inv_num']."','".
							$parm1['qty']."','".
...
//明細
		$i=$m=0; 		
		$op['po_ship_det'] = array();
		
		$q_str="SELECT po_ship_det.*
						FROM  po_ship_det
						WHERE po_ship_det.ship_id='".$op['po_ship']['id']."'								  					  
						ORDER po_ship_det.id";
		...
		while ($det_row = $sql->fetch($q_result)) {
			$op['po_ship_det'][$m]=$det_row;
			$ttl_qty += $det_row['qty'];
			
			# 用來判斷是否超過 po_qty
			$det_row['ship_qty'] = $this->get_ship_qty($det_row['ap_num'],$det_row['mat_id'],$det_row['color'],$det_row['size']);
			...
			$q_str = "SELECT receive_det.id FROM receive_det 
			 			WHERE receive_det.po_id='".$row1['po_id']."' and receive_det.ap_num='".$ap_num['ap_num']."' and receive_det.ship_det_id='".$row1['id']."'";
		...
		{
			$q_str="SELECT ap_det.unit, sum(ap_det.po_qty) as po_qty, ap_det.prics, ap_det.prc_unit, 
										ap_det.mat_cat, ap.currency, ap_det.po_unit, ap.ap_num, ap.po_num, ap.sup_code, ap.toler
							FROM ap_det, ap
							WHERE ap.ap_num = ap_det.ap_num AND																		
									  ap_det.po_spare='".$op['ship'][$i]['po_id']."'								  
						  GROUP BY ap_det.po_spare ";
			...
			if($op['ship'][$i]['mat_cat'] == 'l')
			{
				$q_str = "SELECT bom_lots.color, lots_use.smpl_code, lots_use.lots_code, lots_use.lots_name, 
									...
									WHERE ap_det.bom_id = bom_lots.id AND bom_lots.lots_used_id = lots_use.id AND
												...
												ap_det.mat_cat = 'l' AND ap_det.po_spare='".$op['ship'][$i]['po_id']."'
				...
				$q_str = "SELECT bom_acc.color, acc_use.smpl_code, acc_use.acc_code, acc_use.acc_name, 
									...
									WHERE ap_det.bom_id = bom_acc.id AND bom_acc.acc_used_id = acc_use.id AND
												...
												ap_det.mat_cat = 'a' AND ap_det.po_spare='".$op['ship'][$i]['po_id']."'
...
//特殊採購 主料		
		$q_str="SELECT po_ship_det.*, ap_special.id as s_id, ap_special.unit, ap_special.po_qty, ap_special.prics, 
						...
						WHERE ap.ap_num = ap_special.ap_num AND po_ship_det.po_id = ap_special.id AND 
									ap_special.mat_code = lots.lots_code AND po_ship_det.special = 1 AND
								  ap_special.mat_cat = 'l' AND po_ship_det.ship_id='".$op['po_ship']['id']."'  ORDER BY po_ship_det.ctn";
...
//特殊採購 副料			
		$q_str="SELECT po_ship_det.*, ap_special.id as s_id, ap_special.unit, ap_special.po_qty, ap_special.prics, 
						...
						WHERE ap.ap_num = ap_special.ap_num AND po_ship_det.po_id = ap_special.id AND 
								  ...
								  ap_special.mat_cat = 'a' AND po_ship_det.ship_id='".$op['po_ship']['id']."'  ORDER BY po_ship_det.ctn";
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function del_det($id,$po_id,$special) {
	$sql = $this->sql;

		$q_str="SELECT po_ship_det.ship_id
						FROM   po_ship_det	WHERE po_ship_det.id='".$id."'";
		...
		$q_str="SELECT po_ship.ship_way, po_ship.ship_date, po_ship.ship_eta
						...
						WHERE po_ship_det.po_id='".$po_id."' and po_ship_det.ship_id = po_ship.id order by po_ship_det.ship_id desc limit 1";

		$q_result = $sql->query($q_str);
		$ship_row = $sql->fetch($q_result);
		
		$special == 1 ? $tbl = "ap_special" : $tbl = "ap_det";
		$f1 = $this->update_field("ship_date", $ship_row['ship_date'], "po_spare = '".$po_id."'", $tbl);
		$f1 = $this->update_field("ship_way", $ship_row['ship_way'], "po_spare = '".$po_id."'", $tbl);
		$f1 = $this->update_field("ship_eta", $ship_row['ship_date'], "po_spare = '".$po_id."'", $tbl);
		
		
		$q_str="SELECT id
						FROM   po_ship_det	WHERE po_ship_det.ship_id='".$ship_id."' Limit 1";
		...
		{
	 		$q_str="DELETE FROM po_ship WHERE id='".$ship_id."'";
	 		if (!$q_result = $sql->query($q_str)) {
				$this->msg->add("Error !  Database can't update.");
				$this->msg->merge($sql->msg);
				return false;    
...
function create_html($op)	
{
	...
	{
		$txt1.="<tr>";
		$txt1.="<td  align=left nowrap>&nbsp;".$op['ship'][$i]['ctn']."</td>";
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#	->get_ship_qty($ap_num,$mat_id,$color,$size)
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function get_ship_qty($ap_num,$mat_id,$color,$size){
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#	->get_same_mat($ap_num,$mat_id,$color,$size)
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function get_same_mat($ap_num,$mat_id,$color,$size){
	$sql = $this->sql;
	
	$q_str = "select ap_num, bom_id, mat_cat, mat_id, used_id, color, size, ap_qty, po_qty
			  from ap_det
			  where ap_num = '".$ap_num."' and mat_id = ".$mat_id." and color = '".$color."' and size = '".$size."'";
