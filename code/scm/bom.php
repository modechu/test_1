...
//= trim_add ====================================================
	...
	case "upload_trim_file":
		...
		if ($ext == "pdf" || $ext == "PDF"){   // 上傳檔的副檔名為 mdl 時 -----
			...
			$upload = new Upload;
			$upload->uploadFile('order_trim/', 'other', 16, $pttn_name );
			if (!$upload){
				$op['msg'][] = $upload;				
				$layout->assign($op);
...
$op['factory_select'] = $arry2->select(get_factory_group(),'','PHP_fty_sch','select','');
...
$op['marker_info'] = $order->get('',$op['wi']['wi_num']);
$fy=$op['marker_info']['factory'];
...
//$op['bom_lots_list'][$keys]['length_total'] =0;
//				$op['bom_lots_list'][$keys]['over_marker'] = 0;
if ($op['mks'] = $Marker->get(2,0,$op['marker_info']['id'],1,null,'',1)) {
    foreach($op['mks'] as $keys => $vals){
        if(is_array($vals)){
			...
			foreach($op['bom_lots_list'] as $bom_keys => $bom_vals){
				if($bom_vals['change'] == 0)
				{
					if($vals['mat_code'] == $bom_vals['lots_code'])
					{
						$op['bom_lots_list'][$bom_keys]['length_total'] =0;
						$op['bom_lots_list'][$bom_keys]['over_marker'] = 0;
						$op['bom_lots_list'][$bom_keys]['marker_waste'] = $op['marker_waste_value'];//單拉只有一層
						$op['bom_lots_list'][$bom_keys]['length_total'] = round(($op['mks'][$keys]['averages']+$op['marker_waste_value']),2);
						if($op['bom_lots_list'][$bom_keys]['est_1'] > $op['bom_lots_list'][$bom_keys]['length_total']*1.05)
						{
							$op['bom_lots_list'][$bom_keys]['over_marker'] = 1;
						}
						/*
						$op['bom_lots_list'][$bom_keys]['marker_waste'] = $op['marker_waste_value'];//單拉只有一層
						$op['bom_lots_list'][$bom_keys]['length_total'] = round(($op['mks'][$keys]['averages']+$op['marker_waste_value']),2);
						if($op['bom_lots_list'][$keys]['est_1'] > $op['bom_lots_list'][$keys]['length_total']*1.05)
						{
							$op['bom_lots_list'][$bom_keys]['over_marker'] = 1;
						}
						*/
...
//=upload_bom_file======================================================		
	case "upload_bom_file":
		...
		if ($f_check == 1){   // 上傳檔的副檔名為 mdl 時 -----
			...
			$upload->uploadFile('bom_file/', 'other', 16, $pttn_name );
			$upload->setMaxSize(2072864);
			if (!$upload){
				$op['msg'][] = $upload;
				$layout->assign($op);
...
for($i=0; $i<sizeof($p_id); $i++)
{
	...
	for ($j=0;$j<$num_colors;$j++){					
		...
		$T_qty = explode(",", $T_wiqty[$j]['qty']);
		$op['color'][$k]['qty'] = array_sum($T_qty);
		$op['color'][$k]['pid'] = $p_id[$i];
		$k++;
	}
	// 做出 size table-------------------------------------------
    
	$reply = show_breakdown($T_wiqty,$sizing,$size_A['base_size'],$p_etd[$i],$i,$p_remark[$i],$p_id[$i]);
...
$bom_photo = $bom->get_bom_photo($op['wi']['id']);
// print_r($bom_photo);
// --------------- 主料 ------------ 做出 $bom_lots_list[]  array....
$op['bom_lots_list'] = array();
if (!$op['lots_NONE']){   // 樣本有主料用料記錄---
	$op['bom_lots_list'] =  bom_lots_view($num_lots_used,$bom_lots['lots'],$lots_used['lots_use'],$op['num_colors'],$bom_photo['lots']);
}
...
$op['bom_acc_list'] = array();
if (!$op['acc_NONE']){   // 樣本有副料用料記錄---
	$op['bom_acc_list'] =  bom_acc_view($num_acc_used,$bom_acc['acc'],$acc_used['acc_use'],$op['num_colors'],$bom_photo['acc']);
}
// ---------------------- end BOM data layout [副料] ----------------------------
...
if (!$num_colors){	
	...
	$op['total_fields'] = 10 + $op['num_colors'];
	}
        ...
        if(is_array($vals)){
			...
			foreach($op['bom_lots_list'] as $bom_keys => $bom_vals){
				...
				if($vals['mat_code'] === $bom_vals['lots_code'])
				{
					...
					/*
					$op['bom_lots_list'][$keys]['marker_waste'] = $op['marker_waste_value'];//單拉只有一層
					$op['bom_lots_list'][$keys]['length_total'] = round(($op['mks'][$keys]['averages']+$op['marker_waste_value']),2);
					if($op['bom_lots_list'][$keys]['est_1'] > $op['bom_lots_list'][$keys]['length_total']*1.05)
					{
						$op['bom_lots_list'][$keys]['over_marker'] = 1;
					}
					*/
				}
...
//==bom_consum_add=====================================================
	case "bom_consum_add":
		...
		$acc_use_size = sizeof($op['acc_use']);
		for($i=0;$i<$acc_use_size;$i++){
			$ap_mark = $wi->check_con_po($op['acc_use'][$i]['id']);
			if($ap_mark)
				$op['acc_use'][$i]['ap_mark'] = str_replace("PA","PO",$ap_mark);
		}
//--------------------------------------------------------------------------
		// 取出 size_scale ----------------------------------------------
		$size_A = $size_des->get($op['smpl']['size']);
...
$parm = array($PHP_id,'confirmer',$GLOBALS['SCACHE']['ADMIN']['name']);
$wi->update_field($parm);
$parm = array($PHP_id,'cfm_date',$TODAY);
$wi->update_field($parm);
$parm = array($PHP_id, 'bsub_user', $GLOBALS['SCACHE']['ADMIN']['login_id']);
$wi->update_field($parm);	

$message = "Successfully SUBMIT BOM : [".$PHP_wi_num."]";
$log->log_add(0,"022S",$message);
...
case "upload_fty_pattern":
	...
	$upload = new Upload;
	$upload->uploadFile('fty_pattern/', 'other', 16, $pttn_name );
...
$filename = $mat.'_'.$bom_id;
// $filename = $_GET['mat_code'];
$filename = str_replace('&gt;','',$filename);
$filename = str_replace('&lt;','',$filename);
$ext = strtoupper(preg_replace("/.*\.([^.]+)$/","\\1", $_FILES[$filename]['name']));
// echo $filename;
$error = "";
$msg = "";
...
if(!empty($_FILES[$fileElementName]['error'])){
    ...
    $msg .= " File Size: " . @Size($_FILES[$fileElementName]['tmp_name']). ",";
    //for security reason, we force to remove all uploaded file
    @unlink($_FILES[$fileElementName]);		
}


if(@filesize($_FILES[$fileElementName]['tmp_name']) > 1024000){
    $error .= "Please use the correct file size.(smaller than 1mb)," ;
}

if( $ext != 'JPG' && $ext != 'JPEG' && $ext != 'jpg' && $ext != 'jpeg' ){
    $error .= "Please use the correct file type(jpg)," ;
}

$msgs = '';
if( empty($error) ) {

    $Upload->uploadMat('./images/bom_pic/',$fileElementName );
    if (!$Upload){
        $op['msg'][] = $Upload;				
        $layout->assign($op);
        $layout->display($TPL_ERROR);  		    
    ...
    if(is_file('./images/bom_pic/'.$fileElementName.'.jpg')){
        $parm = array(
            "wi_id"		=>	$wi_id,
            "mat"		=>	substr($mat,0,1),
            "bom_id"    =>	$bom_id,
            "creator"   =>	$GLOBALS['SCACHE']['ADMIN']['id']
        );

        $p_id = $bom->add_bom_photo($parm);
        $msgs = 'success';
        $message = "Uploading success! Mat. images ".'['.$_FILES[$fileElementName]['name'].'],';
        $log->log_add(0,"022U",$message);
    } else {
        $msgs = 'error';
