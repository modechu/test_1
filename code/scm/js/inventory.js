	...
	$(document).ready(function(){
		...
		$('input[class=submit_ajax]').click(function(){
			...
			switch( SubName ){
				...
				if(!num){
                    ...
                    str += '\nSend number# : ' + notice_num ;
				} else {
                    str += '\nOrder number# : ' + num ;
                    str += (notice_num?'\nSend number# : ' + notice_num:'') ;
                }

                if ( confirm('Are you sure to Send '+ str ) ){
                    window.location = "?PHP_action=receiving_to_get_material_view&M_num="+$(this).attr('order_num')+"&M_notice_num="+$(this).attr('notice_num')+"&M_mat="+$(this).attr('mat_cat');
					location.href = "receiving_to_get_material.php?PHP_action=receiving_to_get_material_search&order_num="+num+"&notice_num="+notice_num+"&mat="+mat;
				} else {
					$.unblockUI();
					return false;
				}
		...
		$('#bl_num').live('keyup', function(){
			$.ajax({
				...
				data: {
					bl_num : $('#bl_num').val(),
					arrival_active : $('#arrival_active').val(),
					incoming_active : $('#incoming_active').val(),
		...
		$('#List_TR_link').live('click', function(){
			...
			if( $(this).attr('link') ){
				$.blockUI();
				location.href = "incoming_material.php?PHP_action=incoming_material_search&bl_num="+$(this).attr('link');
				$.unblockUI();
			}
		});
		$('#List_TR_link_arrival').live('click', function(){
			// var order = $("#PHP_order").val();
			// if(!order){
				// alert("Please input order#");
				// return false;
			...
			if( $(this).attr('link') ){
				$.blockUI();
				location.href = "arrival_time.php?PHP_action=arrival_time_search&bl_num="+$(this).attr('link');
				$.unblockUI();
			}
		});
		...
		select_checkbox = function(status){
			if ( status == 'Reply' ) {
				...
				$("input[id^=link_order_qty_]").each(function(i, elm) {
					var id = $(elm).attr('id').replace('link_order_qty_', '');
					$(elm).val($('#TMP_link_order_qty_'+id).val());
				});
			} else {
				$("input[id^=po_ship_link_qty]").each(function(i, elm) {
					$(elm).val('0');
		...
		arrival_chg = function(dept){
			$.blockUI();
            var arrival_active = $('#arrival_active').val();
            var incoming_active = $('#incoming_active').val();
			location.href = "arrival_time.php?PHP_action=arrival_time&PHP_dept="+$(dept).val()+"&arrival_active="+arrival_active+"&incoming_active="+incoming_active;
		}
		...
		incoming_chg = function(dept){
            ...
            var arrival_active = $('#arrival_active').val();
            var incoming_active = $('#incoming_active').val();
			location.href = "incoming_material.php?PHP_action=incoming_material&PHP_dept="+$(dept).val()+"&arrival_active="+arrival_active+"&incoming_active="+incoming_active;
		}
        ...
        $("tr #notice_link").live("click", function () {
           window.location = "?PHP_action=receiving_to_get_material_view&M_num="+$(this).attr('notice_num')+"&M_mat="+$(this).attr('mat_cat');
		});
		...
		show_un_send_list = function(r){
			$(r).css("display", "none");
			$.blockUI();
			location.href = "receiving_to_get_material.php?show_un_send_list=1";
		}
