...
class PowerGraphic {
    ...
    function PowerGraphic()   {
        $this->x = $this->y = $this->z = $this->a = array();

        $this->biggest_x        = NULL;
        $this->biggest_y        = NULL;
        $this->alternate_x      = false;
        $this->graphic_2_exists = false;
        $this->graphic_3_exists = false;    //---- ADD
        $this->graphic_4_exists = false;    //---- ADD
        ...
        $this->graphic_3 = (isset($_GET['graphic_3'])) ? $_GET['graphic_3'] : "";   //---- ADD
        $this->graphic_4 = (isset($_GET['graphic_4'])) ? $_GET['graphic_4'] : "";   //---- ADD
		...
		$bigger_graphic_name = (strlen($this->graphic_1) > strlen($this->graphic_2)) ? $this->graphic_1 : $this->graphic_2;	 //---- ADD
		$bigger_graphic_name = (strlen($bigger_graphic_name) > strlen($this->graphic_3)) ? $bigger_graphic_name : $this->graphic_3;    //---- ADD
        $this->biggest_graphic_name = (strlen($bigger_graphic_name) > strlen($this->graphic_4)) ? $bigger_graphic_name : $this->graphic_4;    //---- ADD
		
		$this->height_title         = (!empty($this->title)) ? ($this->string_height(5) + 15) : 0;
        $this->space_between_bars   = ($this->type == 1) ? 37 : 30;    // 設定bars間 的寬度
    ...
    function start()  {
            ...
            if (preg_match("/^x\d+$/i", $parameter))   {
                if (strtolower($parameter{0}) == 'x')      {
                    ...
                    if ((!empty($_GET['z'.$num])) && (ereg("(1|2|3|4)", $this->type)))  {
						...
						//---- ADD ---- start ----
                    if ((!empty($_GET['a'.$num])) && (ereg("(1|2|3|4)", $this->type)))  {
                        $this->graphic_3_exists = true;
                    }
                    if ((!empty($_GET['b'.$num])) && (ereg("(1|2|3|4)", $this->type)))  {
                        $this->graphic_4_exists = true;
                    }
						//---- ADD ---- end ----
        ...
        // Defines arrays $this->x, $this->y and $this->z (if exists) and $this->a (if exists) and $this->b (if exists)
        foreach ($temp as $index => $parameter)     {
            ...
            if (!empty($_GET['y'.$index]))   {
                ...
                if ($_GET['y'.$index] > $this->biggest_y)   {
                    $this->biggest_y = number_format(round($_GET['y'.$index], 1), 0, ".", "");   //改
                }
            }
            ...
            if ($this->graphic_2_exists)   {
                ...
                if ($value > $this->biggest_y)   {
                    $this->biggest_y = number_format(round($value, 1), 0, ".", "");   //改
                }
            }

						//---- ADD ---- start ----
            if ($this->graphic_3_exists)   {
                $value       = (!empty($_GET['a'.$index])) ? $_GET['a'.$index] : 0;
                $this->a[$i] = $value;
            ...
            if ($this->graphic_4_exists)   {
                $value       = (!empty($_GET['b'.$index])) ? $_GET['b'.$index] : 0;
                $this->b[$i] = $value;
    ...
    function create_graphic() {
        ...
        if (ereg("^(1|3|4)$", $this->type))  {
            ...
            for ($i = 1; $i < 10; $i++)  {
                ...
                if ($i % 2 == 0) {
                    $value = $this->number_formated($this->higher_value * $i / 10, 0);	//改
                    $less = 7 * strlen($value);
        ...
        /**
        * Draw graphic: VERTICAL BARS  ------- 劃出 垂直條狀圖 -------------------
        */
        if ($this->type == 1)  {
            $num = 1;
		...
		//---- ADD ---- start --------------
                if (isset($this->b[$i])) {
                    $size = round($this->b[$i] * $higher_value_size / $this->higher_value);
                    $x1   = $x + 18;
                    $y1   = ($this->graphic_area_y2 - $size) + 1;
                    $x2   = $x1 + 18;
                    $y2   = $this->graphic_area_y2 - 1;
                    imageline($this->img, ($x1+1), ($y1-1), $x2, ($y1-1), $this->color['bars_4_shadow']);
                    imageline($this->img, ($x2+1), ($y1-1), ($x2+1), $y2, $this->color['bars_4_shadow']);
                    imageline($this->img, ($x2+2), ($y1-1), ($x2+2), $y2, $this->color['bars_4_shadow']);
                    imagefilledrectangle($this->img, $x1, $y1, $x2, $y2, $this->color['bars_4']);
                }
		//---- ADD ---- end --------------
		//---- ADD ---- start --------------
                if (isset($this->a[$i])) {
                    $size = round($this->a[$i] * $higher_value_size / $this->higher_value);
                    $x1   = $x + 12;
                    $y1   = ($this->graphic_area_y2 - $size) + 1;
                    $x2   = $x1 + 18;
                    $y2   = $this->graphic_area_y2 - 1;
                    imageline($this->img, ($x1+1), ($y1-1), $x2, ($y1-1), $this->color['bars_3_shadow']);
                    imageline($this->img, ($x2+1), ($y1-1), ($x2+1), $y2, $this->color['bars_3_shadow']);
                    imageline($this->img, ($x2+2), ($y1-1), ($x2+2), $y2, $this->color['bars_3_shadow']);
                    imagefilledrectangle($this->img, $x1, $y1, $x2, $y2, $this->color['bars_3']);
                }
		//---- ADD ---- end --------------
...
function calculate_width() {
    ...
    switch ($this->type) {
        ...
        case 1:
            ...
            $this->width += ($this->legend_exists == true) ? 50 : ((5 * strlen($this->axis_x)) + 10);
            break;
    ...
    function draw_legend()  {
        ...
        if (ereg("^(1|2|3|4)$", $this->type)) {
            ...
            $color_3 = (ereg("^(1|2)$", $this->type)) ? $this->color['bars_3'] : $this->color['line_3'];  //---- ADD 
            $color_4 = (ereg("^(1|2)$", $this->type)) ? $this->color['bars_4'] : $this->color['line_4'];  //---- ADD 
            ...
            $y += 20;
            imagefilledrectangle($this->img, $x, $y, ($x+10), ($y+10), $color_3);
            imagerectangle($this->img, $x, $y, ($x+10), ($y+10), $this->color['title']);
            imagestring($this->img, 3, ($x+15), ($y-2), $this->graphic_3, $this->color['axis_values']);
            $y += 20;
            imagefilledrectangle($this->img, $x, $y, ($x+10), ($y+10), $color_4);
            imagerectangle($this->img, $x, $y, ($x+10), ($y+10), $this->color['title']);
            imagestring($this->img, 3, ($x+15), ($y-2), $this->graphic_4, $this->color['axis_values']);
        }
    ...
    function number_formated($number, $dec_size = 0)  {    //改
        ...
        switch ($this->skin)   {
            ...
            case 1:
                ...
                if (ereg("^(1|2)$", $this->type))
                {
                    ...
                    $this->color['bars_2']        = imagecolorallocate($this->img, 255, 190, 100);
                    $this->color['bars_2_shadow'] = imagecolorallocate($this->img, 100, 100, 100);
					//---- ADD ---- start ----
					$this->color['bars_3']        = imagecolorallocate($this->img, 180, 125, 115);
                    $this->color['bars_3_shadow'] = imagecolorallocate($this->img, 100, 100, 100);
					//---- ADD ---- end ----
					//---- ADD ---- start ----
//					$this->color['bars_4']        = imagecolorallocate($this->img, 205, 205, 190);
					$this->color['bars_4']        = imagecolorallocate($this->img, 255, 60, 60);
                    $this->color['bars_4_shadow'] = imagecolorallocate($this->img, 100, 100, 100);
					//---- ADD ---- end ----
                }
                else if (ereg("^(3|4)$", $this->type))
                {
                    ...
                    $this->color['line_3'] = imagecolorallocate($this->img, 205, 120, 100);
                }
                else if (ereg("^(5|6)$", $this->type))
                {
            ...
            case 2:
                ...
                if (ereg("^(1|2)$", $this->type))
                {
                    ...
                    $this->color['bars_3']        = imagecolorallocate($this->img, 255, 255, 255);
                    $this->color['bars_3_shadow'] = imagecolorallocate($this->img, 220, 220, 220);
                    $this->color['bars_4']        = imagecolorallocate($this->img, 255, 255, 255);
                    $this->color['bars_4_shadow'] = imagecolorallocate($this->img, 220, 220, 220);
                }
                else if (ereg("^(3|4)$", $this->type))
                {
                    $this->color['line']   = imagecolorallocate($this->img, 220, 220, 220);
                    $this->color['line_2'] = imagecolorallocate($this->img,   0, 180,   0);
                    $this->color['line_3'] = imagecolorallocate($this->img, 100, 180,   0);
                }
                else if (ereg("^(5|6)$", $this->type))
                {
            ...
            case 3:
                ...
                if (ereg("^(1|2)$", $this->type))
                {
					...
					//---- ADD ---- start ----
					$this->color['bars_3']        = imagecolorallocate($this->img, 200, 250, 150);
                    $this->color['bars_3_shadow'] = imagecolorallocate($this->img, 120, 170,  70);
					//---- ADD ---- end ----
 					//---- ADD ---- start ----
					$this->color['bars_4']        = imagecolorallocate($this->img, 200, 250, 150);
                    $this->color['bars_4_shadow'] = imagecolorallocate($this->img, 120, 170,  70);
					//---- ADD ---- end ----
               }
                else if (ereg("^(3|4)$", $this->type))
                {
                    ...
                    $this->color['line_2'] = imagecolorallocate($this->img, 200, 220,  50);
                }
                else if (ereg("^(5|6)$", $this->type))
                {
                    $this->color['arc_1']        = imagecolorallocate($this->img, 100, 150, 200);
...
function add_parameter() {
    ...
    new_line += " <input type=\"text\" name=\"z" + i + "\" id=\"z" + i + "\" size=\"10\" onkeypress=\"return numbers();\" ";
    new_line += " <input type=\"text\" name=\"a" + i + "\" id=\"a" + i + "\" size=\"10\" onkeypress=\"return numbers();\" ";
	new_line += " <input type=\"text\" name=\"b" + i + "\" id=\"b" + i + "\" size=\"10\" onkeypress=\"return numbers();\" style=\"display: " + display_status + ";\" /> </div>";
...
function show_graphic_3_values() {
    var display_status = (document.form_data.graphic_3_values.checked == true) ? "" : "none";
    document.getElementById("value_3").style.display = display_status;
    for (var x = 0; x <= i; x++) {
        document.getElementById("a" + x).style.display = display_status;
    }
}

function show_graphic_4_values() {
    var display_status = (document.form_data.graphic_4_values.checked == true) ? "" : "none";
    document.getElementById("value_4").style.display = display_status;
    for (var x = 0; x <= i; x++) {
        document.getElementById("b" + x).style.display = display_status;
    }
}
...
<form action="' . $_SERVER['SCRIPT_NAME'] . '" method="get" name="form_data" id="form_data"> <br />
  ...
  <div style="margin: 10px 0px 10px 0px;">
    ...
    Graphic 3: <input type="text" name="graphic_3" id="graphic_3" style="width: 172px;" /> <br />
    Graphic 4: <input type="text" name="graphic_4" id="graphic_4" style="width: 172px;" /> <br />
  </div>
  ...
  <div style="margin-top: 20px;" id="parameters">
    ...
    <span style="margin-left: 50px;">Value 1</span>  <span id="value_2" style="display: none; margin-left: 25px;">Value 2</span>';
        for ($i = 0; $i <= 4; $i++) {
            ...
            echo ' <input type="text" name="z'.$i.'" id="z'.$i.'" size="10" onkeypress="return numbers();" /> ';
            echo ' <input type="text" name="a'.$i.'" id="a'.$i.'" size="10" onkeypress="return numbers();" /> ';
            echo ' <input type="text" name="b'.$i.'" id="b'.$i.'" size="10" onkeypress="return numbers();" style="display: none;" /> </div>';
        }
echo 
'    <div id="add_parameter"></div>
  ...
  <input type="submit" value="Create" style="cursor: pointer; margin: 10px 0px 0px 60px;" />
    ...
    function reset_values() {
        ...
        $this->graphic_4 = NULL;
        $this->credits   = NULL;
        $this->x = $this->y = $this->z = $this->a = $this->b = array();
    }
    ...
    function create_query_string() {
        ...
        $graphic['graphic_3'] = $this->graphic_3;   //---- ADD ----
        $graphic['graphic_4'] = $this->graphic_4;   //---- ADD ----
        ...
        foreach ($this->x as $i => $x)  {
            ...
            if (isset($this->a[$i])) { $graphic['a'.$i] = $this->a[$i]; }   //---- ADD ----
            if (isset($this->b[$i])) { $graphic['b'.$i] = $this->b[$i]; }   //---- ADD ----
        }
