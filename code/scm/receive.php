...
if($dept=='LY'||$dept=='HJ'||$dept=='CF')
{
	$op['fty']="<input type='text' id='PHP_ship' name='PHP_ship' value='".$dept." readonly'>";
}
else
{
...
$op['rcvd'] = $receive->get_receive_check_list();

page_display($op, '043', $TPL_RCVD);
break;
...
$PHP_po = (!empty($_POST['PHP_po'])?$_POST['PHP_po']:(!empty($_GET['PHP_po'])?$_GET['PHP_po']:''));
$PHP_ship = (!empty($_POST['PHP_ship'])?$_POST['PHP_ship']:(!empty($_GET['PHP_ship'])?$_GET['PHP_ship']:''));
$PHP_ord = (!empty($_POST['PHP_ord'])?$_POST['PHP_ord']:(!empty($_GET['PHP_ord'])?$_GET['PHP_ord']:''));
$PHP_ship2 = (!empty($_POST['PHP_ship2'])?$_POST['PHP_ship2']:(!empty($_GET['PHP_ship2'])?$_GET['PHP_ship2']:''));
$PHP_sup = (!empty($_POST['PHP_sup'])?$_POST['PHP_sup']:(!empty($_GET['PHP_sup'])?$_GET['PHP_sup']:''));
$PHP_inv_num = (!empty($_POST['PHP_inv_num'])?$_POST['PHP_inv_num']:(!empty($_GET['PHP_inv_num'])?$_GET['PHP_inv_num']:''));

if($PHP_ord <> ''){
    //$op = $receive->search_ord();
}elseif($PHP_po <> ''){
    $op = $receive->search_po();
}else{
    $op = $receive->search_inv();
}

$op['back_str'] = "&PHP_po=".$PHP_po."&PHP_ship=".$PHP_ship."&PHP_ship2=".$PHP_ship2."&PHP_sup=".$PHP_sup."&PHP_ord=".$PHP_ord."&PHP_inv_num=".$PHP_inv_num;

page_display($op, '043', $TPL_RCV_PO_LIST);
break;		
...
$op = $poship->get('',$PHP_invoice);

for ($i =0; $i< sizeof($op['ship']); $i++)
{
	$rcv_row = $receive->get_det_field("sum(qty) as qty","receive,receive_det","ship_det_id=".$op['ship'][$i]['id']." and receive.rcv_num=receive_det.rcv_num and receive.tw_rcv=0");
	$op['ship'][$i]['rcv_qty'] = $rcv_row['qty'];
	$op['ship'][$i]['un_qty'] = $op['ship'][$i]['qty'] - $op['ship'][$i]['rcv_qty'];
	$op['ship'][$i]['i'] = $i;
}
...
$op['date'] = $TODAY;
$op['back_str'] = "&PHP_po=".$PHP_po."&PHP_ship=".$PHP_ship."&PHP_sup=".$PHP_sup."&PHP_sr_startno=".$PHP_sr_startno."&PHP_ord=".$PHP_ord;

page_display($op, '043', $TPL_RCV_ADD);
break;
...
if (isset($PHP_mat_code)) {
    for ($i = 0; $i < sizeof($PHP_mat_code); $i++) {
        if ($PHP_rcv[$i] > 0) $add_mk = 1;
    }
}

if($add_mk == 0) {
    $msg = "Please input Receiving Q'TY！";
    redirect_page($PHP_SELF."?PHP_action=rcvd_add&PHP_invoice=".$PHP_ship_inv."&PHP_msg=".$msg);
}

//if($PHP_inv){$inv_date = $TODAY;}else{$inv_date = '0000-00-00';}
$parm = array (	
    "ship_num"	=>	$PHP_ship_num,
    "rcv_date"	=>	$TODAY,
    ...
    for ($i = 0; $i < sizeof($PHP_mat_code); $i++) {
        if ($PHP_rcv[$i] > 0) {
            ...
            for ($j=0; $j< sizeof($id); $j++)		//取得每項的請購量,並加總
            {
                ...
                //$row2[$j]=$po->get_det_field('rcv_qty','ap_det',$where_str);
                $row2[$j]['rcv_qty']=$receive->get_rcvd_qty_by_spec($PHP_inv[$i],$id,$PHP_special[$i]);
                $row[$j]['po_qty'] -= $row2[$j]['rcv_qty'];
                if($row[$j]['po_qty'] < 0)$row[$j]['po_qty'] = 0;
                $s_total += $row[$j]['po_qty'];
            }		

            if($PHP_po_unit[$i] == 'K pcs')
            {
                $chg_qty = $PHP_rcv[$i] * 1000;
                $div = 1000;
            }elseif($PHP_po_unit[$i] == '100pcs'){
                $chg_qty = $PHP_rcv[$i] * 100;
                $div = 100;		
            }elseif($PHP_po_unit[$i] == 'dz'){
                $chg_qty = $PHP_rcv[$i] * 12;
                $div = 12;		
            }elseif($PHP_po_unit[$i] == 'gross'){
                $chg_qty = $PHP_rcv[$i] * 144;
                $div = 144;
            }else{
                $chg_qty = $PHP_rcv[$i];
                $div = 1;		
            ...
            {
                $rcv_qty[$j] = $chg_qty * ($row[$j]['po_qty'] / $s_total);
                $rcv_qty[$j] = number_format($rcv_qty[$j],0,'.','');
                $rcv_qty[$j] = number_format(($rcv_qty[$j]/$div),2,'.','');
            ...
            $tbl = $PHP_special[$i] == 1 ? 'ap_special' : 'ap_det';
            for ($j=0; $j< sizeof($id); $j++)						//加入DB
            {
                $ord_num = $po->get_ord_num($id[$j],$PHP_special[$i],$PHP_mat_code[$i]);		
                $f3=$receive->add_link($f1,$id[$j],$rcv_qty[$j],$ord_num);
                $f2=$receive->update_rcv_qty('rcv_qty',$rcv_qty[$j], $id[$j],$tbl);	
            }

            //超量備註
            if($PHP_reason[$i])
            {
                $parm= array(	'rcv_num'		=>	$parm['rcv_num'],
                                'item'			=>	'Overbalance.',
                                'des'				=>	$PHP_reason[$i],
                                'user'			=>	$GLOBALS['SCACHE']['ADMIN']['login_id'],
                                'k_date'		=>	date('Y-m-d')
                );
                $f4=$receive->add_log($parm);
            }
        }
    }
}
...
/* if (isset($PHP_s_mat_code))
{
    for ($i = 0; $i < sizeof($PHP_s_mat_code); $i++)
    {
        if ($PHP_s_rcv[$i] > 0)
        {
            ...
            $f1 = $receive->add_det($parm_det);						
            $ord_num = $po->get_ord_num($PHP_s_id[$i],1,$PHP_s_mat_code[$i]);	
            $f3=$receive->add_link($f1,$PHP_s_id[$i],$PHP_s_rcv[$i],$ord_num);
            $f2 = $receive->update_rcv_qty('rcv_qty',$PHP_s_rcv[$i], $PHP_s_id[$i],'ap_special');	
...
//	$f1 = $receive->check_ord_rcvd($parm['rcv_num'],0);
...
$op['msg'][]= $message;
$log->log_add(0,"043A",$message);
//echo $PHP_SELF."?PHP_action=rcvd_view&PHP_rcv_num=".$parm['rcv_num']."&SCH_num=&SCH_supl=&SCH_fab=&SCH_acc=&PHP_sr_startno=1";exit;
redirect_page($PHP_SELF."?PHP_action=rcvd_view&PHP_rcv_num=".$parm['rcv_num']."&SCH_num=&SCH_supl=&SCH_fab=&SCH_acc=&PHP_sr_startno=1");
#page_display($op, '043', $TPL_RCV_SHOW);
break;
...
$op['back_str'] = "&SCH_num=".$SCH_num."&SCH_supl=".$SCH_supl."&SCH_po=".$SCH_po."&SCH_order=".$SCH_order."&PHP_ship=".$PHP_ship;
$op['msg'] = $receive->msg->get(2);
if(isset($PHP_msg)) $op['msg'][] = $PHP_msg;
page_display($op, '043', $TPL_RCV_LIST);
break;				
...
if( $op['rcv']['rcv_cfm_user'] == $GLOBALS['SCACHE']['ADMIN']['login_id'] or $GLOBALS['SCACHE']['ADMIN']['dept'] == "SA")
	$op['manager_flag'] = 1;
...
page_display($op, '043', $TPL_RCV_EDIT);
break;
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    case "do_rcvd_edit":
		...
		if($PHP_del_mk == 1)
		{
			...
			$redir_str = "receive.php?PHP_action=rcvd_search".$PHP_back_str."&PHP_msg=".$message;
...
//										"tax"				=>	$PHP_tax,
		...
		for ($i = 0; $i < sizeof($PHP_qty); $i++)
		{
			if (isset($PHP_qty[$i]) && isset($PHP_rcv_qty[$i]))
			{
				...
				if ($qty  <> 0)//原本是用 $qty <> 0(數量有更改)才更新資料庫, 但rcv_po_link 的 qty有零,所以改成每筆都更新
				{
					...
					if ($PHP_po_cat[$i] < 2)
					{
						...
						for ($j=0; $j< (sizeof($id)-1); $j++)		//計算每項採購量(平分)
						{
							$rcv_qty[$j] = $PHP_qty[$i] * ($row[$j]['po_qty'] / $s_total);
							$rcv_qty[$j] = number_format($rcv_qty[$j],0,'.','');
							$tmp = $tmp + $rcv_qty[$j];
						}	
						$tmp_qty =  $PHP_qty[$i] - $tmp;
						$rcv_qty[$j] = $tmp_qty;
						$rcv_qty[$j] = number_format($rcv_qty[$j],2,'.','');
						for ($j=0; $j< sizeof($id); $j++)								//加入DB
						{
							$f1=$receive->update_link_qty($PHP_det_id[$i],$id[$j],$rcv_qty[$j]);
							$f1=$receive->update_rcv_qty('rcv_qty',$rcv_qty[$j], $id[$j],'ap_det');	
												
						}
					}else{			
						$f1=$receive->update_link_qty($PHP_det_id[$i],$PHP_po_id[$i],$PHP_qty[$i]);
...
$op['msg'][]=$message;
//if ($PHP_revise == 1)
//	page_display($op, '043', $TPL_RCV_REVISE);
//else
// print_r($PHP_rcv_qty);
// print_r($PHP_qty);
	page_display($op, '043', $TPL_RCV_SHOW);
...
/*
					...
					$rcv_qty[$j] = number_format($rcv_qty[$j],2,'.','');
*/
				//$ap_table = $receive->get_table_name($PHP_id);
				if($PHP_special < 2)
				{
					$qty_fd = $receive->get_link_field('po_id, qty','rcv_po_link','rcv_id ='.$PHP_id);
					$f1 = $receive->del_link($PHP_id);
					for ($j=0; $j< sizeof($qty_fd); $j++)		//加入DB
					{
						$f1=$receive->update_rcv_qty('rcv_qty',($qty_fd[$j]['qty']*-1), $qty_fd[$j]['po_id'],'ap_det');	# 不再寫入ap_det的rcv_qty
					}
					if(substr($PHP_mat_code,0,1) == 'F')
					{
						$ord_rec = $receive->get_ord_rec($qty_fd,'l','ap_det');						
						$f1 = $receive->back_ord_rcvd('l',$ord_rec);
					}else{
						$ord_rec = $receive->get_ord_rec($qty_fd,'a','ap_det');			
						$f1 = $receive->back_ord_rcvd('l',$ord_rec);
					}
				}else{
					$f1 = $receive->del_link($PHP_id);
					...
					if(substr($PHP_mat_code,0,1) == 'F')
					{
						...
						$ord_rec = $receive->get_ord_rec($qty_fd,'l','ap_special');						
						$f1 = $receive->back_ord_rcvd('l',$ord_rec);
					}else{
						$ord_rec = $receive->get_ord_rec($qty_fd,'a','ap_special');			
						$f1 = $receive->back_ord_rcvd('l',$ord_rec);
					}
				}
				$f1 = $receive->del_det($PHP_id);
				$f2 = $receive->chk_del_full($PHP_rcv_num);
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    case "rcvd_submit":
		...
		$f1 = $receive->change_currency($PHP_rcv_num);
		$f1 = $receive->check_ord_rcvd($PHP_rcv_num,$PHP_ord_num,$PHP_mat_cat);
...
//		$f1 = $receive->check_po_rcvd($PHP_rcv_num);
	...
	/*	
		for($i=0; $i<sizeof($PHP_ord_num); $i++)
		{
			if($PHP_mat_cat == 'l')
			{
				$rcv_date = $order->get_field_value('mat_shp','',$PHP_ord_num[$i],'pdtion');
				if($rcv_date && $rcv_date <> '0000-00-00')
				{
					$order->update_field_num('mat_shp',$TODAY, $PHP_ord_num[$i],'pdtion');
				}else{
					$ord_num[] = $PHP_ord_num[$i];
					$mk = 1;
				}
			}else{
				$macc_date = $order->get_field_value('m_acc_shp','',$PHP_ord_num[$i],'pdtion');
				if($macc_date && $macc_date <> '0000-00-00')
				{
					$order->update_field_num('mat_shp',$TODAY, $PHP_ord_num[$i],'pdtion');
				}
				$acc_date = $order->get_field_value('acc_shp','',$PHP_ord_num[$i],'pdtion');
				if($acc_date && $acc_date <> '0000-00-00')
				{
					$order->update_field_num('acc_shp',$TODAY, $PHP_ord_num[$i],'pdtion');
				}
		...
		*/ 
		...
		for($i=0; $i<sizeof($PHP_ord_num); $i++)
		{
			if($PHP_mat_cat == 'l')
			{
				...
				{
					$receive->add_ord_cost($PHP_ord_num[$i],'l');
				}
			}else{		
				$macc_date = $order->get_field_value('m_acc_shp','',$PHP_ord_num[$i],'pdtion');
				$acc_date = $order->get_field_value('acc_shp','',$PHP_ord_num[$i],'pdtion');
				if(($macc_date && $macc_date <> '0000-00-00') && ($acc_date && $acc_date <> '0000-00-00'))			
				{
					$receive->add_ord_cost($PHP_ord_num[$i],'a');
				}
			}
...
// $f1 = $receive->check_ord_un_rcvd($PHP_rcv_num);

$op = $receive->get($PHP_rcv_num);
foreach($op['ord_num'] as $key){
    # RESET PO STATUS FOR PDTION
    $GLOBALS['order']->reset_pdtion($key['mat_cat'],$key['wi_id']);
}
...
$op['rcvd']=$receive->get_revd_det($PHP_id,$PHP_ap_num);

page_display($op, '043', $TPL_RCV_RECV_SHOW);
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    case "rcv_rpt":
		...
		page_display($op, '043', $TPL_RCVD_RPT);
		break;	
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	if($rcv)
	{
		...
		{
			page_display($op, '043', $TPL_RCV_RPT_USED);
			break;			
...
*/		
	  ...
	  for($k=0; $k<sizeof($ary_unit); $k++)
	  {
		...
		if(!isset($PHP_pdf) && !isset($PHP_excel))
		{
			page_display($op, '043', $TPL_RCV_RPT_DTL);
			break;					
...
$op['rcvd'] = $receive->get_rcv_det($PHP_bom_id,$PHP_ord_num);
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  case "rcvd_cfm_search":
	...
	page_display($op, '73', $TPL_RCV_CFM_LIST);
	break;
...
page_display($op, '076', $TPL_RCV_CFM_SHOW);
break;
...
$f1 = $receive->update_field_id('rcv_cfm_user',$GLOBALS['SCACHE']['ADMIN']['login_id'],$PHP_id,'receive');
$f1 = $receive->update_field_id('rcv_cfm_date',$TODAY,$PHP_id,'receive');
$f1 = $receive->update_field_id('status','4',$PHP_id,'receive');
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  case "reject_rcvd_cfm":
	...
	$f1=$receive->update_field_id('status', '13', $PHP_id);
	$f1=$receive->update_field_id('au_date', $TODAY, $PHP_id);
	$f1=$receive->update_field_id('au_user',$GLOBALS['SCACHE']['ADMIN']['login_id'], $PHP_id);
